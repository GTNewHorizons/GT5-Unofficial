...
#     which is deliberate: the sentinel must stay a miss for by-name resolution so a foreign `dustNULL` entry
#     cannot resolve onto it. Membership is a separate question -- the sentinel IS a declared material, and
#     `LoaderGTBlockFluid`'s autogenerated-plasma pass depends on `isLegacyNamed` answering true for it, so
#     `LegacyNameDomain.contains` covers it explicitly while `lookup` still misses.
#
