...
from gen_materials import BLOCK_CUTOVER_EXCLUDED, GTPP_UNSUPPORTED_PREFIXES
...
# gtpp-only materials (stage 11) declare TextureSet.of("miscutils", <set>) -- a distinct domain from every set
# converted above, even for a set name identical to one gregtech already has (e.g. "METALLIC"): MaterialLib
# resolves a shape's texture under the material's OWN domain's `materials/<set>/` tree, so `miscutils` needs
# its own copy. gtpp never had its own source art for the ~22 sets it reuses (its legacy rendering referenced
# gregtech's `Textures.ItemIcons`/`Textures.BlockIcons` directly -- see `BlockBaseModular#registerBlockIcons`)
# -- including, surprisingly, the five gtpp-exclusive `CUSTOM/<name>` sets (astraltitanium, celestialtungsten,
# chromaticglass, hypogen, rhugnor), which already have real per-material art converted under gregtech's own
# `materials/CUSTOM/<name>/` tree (apparently committed there historically even though only gtpp materials use
# them) -- so every gtpp set's source is gregtech's ALREADY-CONVERTED `materials/<set>/` tree (not the raw
# `materialicons/` legacy slots convert_set reads from), copied byte-for-byte into the miscutils domain root.
MISCUTILS_ASSETS_ITEMS = REPO_ROOT / "src/main/resources/assets/miscutils/textures/items"
MISCUTILS_ITEMS_DEST_ROOT = MISCUTILS_ASSETS_ITEMS / "materials"
MISCUTILS_ASSETS_BLOCKS = REPO_ROOT / "src/main/resources/assets/miscutils/textures/blocks"
MISCUTILS_BLOCKS_DEST_ROOT = MISCUTILS_ASSETS_BLOCKS / "materials"
...
def load_gtpp_new_materials():
    """Parses `gtpp-merge-report.txt`'s "New declarations" section (regenerated by `gen_materials.py`) for
    every gtpp-only material's `textureSet` and dumped `parts` -- the ground truth for which (set, prefix)
    combinations actually need miscutils-domain art. A same-name merge material never uses
    `TextureSet.of("miscutils", ...)` (see this module's header comment), so is deliberately excluded here;
    only entries in this section do."""
    materials = {}
    in_new_section = False
    pattern = re.compile(r"^\s+(\S+): textureSet=(\S+), parts \[(.*)\]\s*$")
    with open(GTPP_MERGE_REPORT_PATH, encoding="utf-8") as f:
        for line in f:
            if line.startswith("== New declarations"):
                in_new_section = True
                continue
            ...
            if not match:
                continue
            name, texture_set, parts_str = match.groups()
            parts = [p.strip("' ") for p in parts_str.split(",")] if parts_str else []
            materials[name] = (texture_set, parts)
    return materials


def load_gtpp_ore_materials():
    """The gtpp-only material names with a legacy ore block (`gtpp-ores.json`, see
    `MaterialDataDump#dumpGtppOres`) -- all 51 are gtpp-only (`load_gtpp_new_materials`), never a name-merge
    (see `GTPPOreAdapter`'s javadoc: a merge material's ore is owned by `GTOreAdapter`, so gtpp never grants
    it `Materials2OreShapes#shapeOre` membership on the gtpp side of a merge, only on a pure-new material)."""
    with open(GTPP_ORES_DUMP_PATH, encoding="utf-8") as f:
        entries = json.load(f)
    return {e["unlocalizedName"] for e in entries}


def gtpp_sets_by_prefix(gtpp_materials, ore_material_names):
    """Aggregates `load_gtpp_new_materials`'s per-material (set, parts) pairs into the reverse mapping this
    module's per-set copy loop needs: `{set_name: {prefix, ...}}`, with a synthetic `"ore"` prefix added for
    every set that has at least one material with a legacy ore block."""
    sets = {}
    for name, (texture_set, parts) in gtpp_materials.items():
        prefixes = sets.setdefault(texture_set, set())
        prefixes.update(parts)
        if name in ore_material_names:
            prefixes.add("ore")
    return sets
...
def convert_gtpp_items(gtpp_sets):
    ...
    shapes_with_texture = set()
    shapes_needed = set()
    for set_name, prefixes in gtpp_sets.items():
        source_dir = DEST_ROOT / set_name
        dest_dir = MISCUTILS_ITEMS_DEST_ROOT / set_name
...
def convert_gtpp_blocks(gtpp_sets):
    """As `convert_gtpp_items`, for the block-kind `block` (storage block) and `ore` shapes, sourced from
    `BLOCK_DEST_ROOT` (the blocks-tree equivalent of `DEST_ROOT`) instead. `oreSmall` is never needed (gtpp
    ore has no small-ore variant, see `GTPPOreAdapter`)."""
    files_written = 0
    block_sets_with_texture = set()
    ore_sets_with_texture = set()
    for set_name, prefixes in gtpp_sets.items():
        source_dir = BLOCK_DEST_ROOT / set_name
        dest_dir = MISCUTILS_BLOCKS_DEST_ROOT / set_name
        for prefix, tracker in (("block", block_sets_with_texture), ("ore", ore_sets_with_texture)):
            ...
            for suffix in (f"{prefix}.png", f"{prefix}.png.mcmeta", f"{prefix}_OVERLAY.png",
                ...
                files_written += 1
                if suffix == f"{prefix}.png":
                    tracker.add(set_name)
    return files_written, block_sets_with_texture, ore_sets_with_texture
...
def main():
    ...
    gtpp_materials = load_gtpp_new_materials()
    gtpp_ore_materials = load_gtpp_ore_materials()
    gtpp_sets = gtpp_sets_by_prefix(gtpp_materials, gtpp_ore_materials)
    gtpp_item_files, gtpp_shapes_needed, gtpp_shapes_with_texture = convert_gtpp_items(gtpp_sets)
    total_files += gtpp_item_files
    gtpp_block_files, gtpp_block_sets_with_texture, gtpp_ore_sets_with_texture = convert_gtpp_blocks(gtpp_sets)
    total_files += gtpp_block_files
    gtpp_sets_needing_block = {s for s, p in gtpp_sets.items() if "block" in p}
    gtpp_sets_needing_ore = {s for s, p in gtpp_sets.items() if "ore" in p}
    gtpp_shapes_never_textured = sorted(gtpp_shapes_needed - gtpp_shapes_with_texture)
    ...
    print(
        f"gtpp (miscutils domain): {len(gtpp_sets)} texture sets used by gtpp-only materials, "
        f"{gtpp_item_files + gtpp_block_files} files written ({gtpp_item_files} item, {gtpp_block_files} block)")
    print(f"  sets needing a block shape: {len(gtpp_sets_needing_block)}, with art: "
          f"{len(gtpp_block_sets_with_texture)} ({sorted(gtpp_sets_needing_block - gtpp_block_sets_with_texture)} "
          "missing)")
    print(f"  sets needing an ore shape: {len(gtpp_sets_needing_ore)}, with art: "
          f"{len(gtpp_ore_sets_with_texture)} ({sorted(gtpp_sets_needing_ore - gtpp_ore_sets_with_texture)} "
          "missing)")
    print(f"  item shapes needed: {len(gtpp_shapes_needed)}, with art in at least one set: "
          f"{len(gtpp_shapes_with_texture)}")
    if gtpp_shapes_never_textured:
        print(f"  item shapes with no source art in ANY needed set: {len(gtpp_shapes_never_textured)}")
        for name in gtpp_shapes_never_textured:
            print(f"    {name}")
