...
public class GregtechMetaPipeEntity_Cable extends GT_MetaPipeEntity_Cable implements IMetaTileEntityCable {
    ...
    private ITexture[] getTextureGTNH(
        ...
        if (aConnected) {
            ...
            return new ITexture[] {
                    TextureFactory.of(wireMaterial.mIconSet.mTextures[TextureSet.INDEX_wire], vRGB),
                    TextureFactory.of(
    ...
    private ITexture[] getTexturePure(
        ...
        if (!(this.mInsulated))
            return new ITexture[] {
                TextureFactory.of(wireMaterial.mIconSet.mTextures[69], Dyes.getModulation(aColorIndex, this.vRGB))
            };
        if (aConnected) {
            float tThickNess = getThickNess();
            if (tThickNess < 0.124F)
            ...
            if (tThickNess < 0.374F)
                return new ITexture[] {
                    TextureFactory.of(wireMaterial.mIconSet.mTextures[69], this.vRGB),
                    TextureFactory.of(
                            Textures.BlockIcons.INSULATION_TINY,
                            Dyes.getModulation(aColorIndex, Dyes.CABLE_INSULATION.mRGBa))
                };
