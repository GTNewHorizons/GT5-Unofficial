        ...
        if (material.getPlasma() != null) {
            ...
            if (ItemUtils.checkForInvalidItems(new ItemStack[] { aPlasmaCell, aContainerItem })) {
                switch (material.getUnlocalizedName()) {
                    case "Runite" -> GTValues.RA.stdBuilder().itemInputs(GTUtility.copyAmount(1L, aPlasmaCell))
                            .itemOutputs(aContainerItem).metadata(FUEL_VALUE, 350_000)
                            .metadata(FUEL_TYPE, GTRecipeConstants.FuelType.PlasmaTurbine.ordinal()).duration(0).eut(0)
                            .addTo(GTRecipeConstants.Fuel);
                    case "CelestialTungsten" -> GTValues.RA.stdBuilder()
                            .itemInputs(GTUtility.copyAmount(1L, aPlasmaCell)).itemOutputs(aContainerItem)
                            .metadata(FUEL_VALUE, 720_000)
                            .metadata(FUEL_TYPE, GTRecipeConstants.FuelType.PlasmaTurbine.ordinal()).duration(0).eut(0)
                            .addTo(GTRecipeConstants.Fuel);
                    default -> GTValues.RA.stdBuilder().itemInputs(GTUtility.copyAmount(1L, aPlasmaCell))
            ...
            if (ItemUtils.checkForInvalidItems(new ItemStack[] { aCell, aPlasmaCell })) {
                GTValues.RA.stdBuilder().itemInputs(aPlasmaCell).itemOutputs(aCell)
                        .duration(Math.max(material.getMass() * 2L, 1L)).eut(TierEU.RECIPE_MV)
