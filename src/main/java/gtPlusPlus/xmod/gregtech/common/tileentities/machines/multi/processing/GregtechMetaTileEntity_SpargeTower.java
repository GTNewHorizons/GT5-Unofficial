...
public class GregtechMetaTileEntity_SpargeTower extends GregtechMeta_MultiBlockBase<GregtechMetaTileEntity_SpargeTower>
    ...
    protected final List<List<Hatch_Output>> mOutputHatchesByLayer = new ArrayList<>();
    ...
    protected boolean addLayerOutputHatch(IGregTechTileEntity aTileEntity, int aBaseCasingIndex) {
        if (aTileEntity == null || aTileEntity.isDead()
            || !(aTileEntity.getMetaTileEntity() instanceof Hatch_Output tHatch)) {
    ...
    public boolean onPlungerRightClick(EntityPlayer aPlayer, ForgeDirection side, float aX, float aY, float aZ) {
        ...
        for (List<Hatch_Output> layer : this.mOutputHatchesByLayer) {
            int aHatchIndex = 0;
            for (Hatch_Output hatch : layer) {
