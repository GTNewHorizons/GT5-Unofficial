...
public abstract class MTELargerTurbineBase extends GTPPMultiBlockBase<MTELargerTurbineBase>
        ...
        protected IStructureDefinition<MTELargerTurbineBase> computeValue(Class<?> type) {
            return StructureDefinition.<MTELargerTurbineBase>builder()
                    ...
                    .addShape(
                            ...
                            (new String[][] {
                                    { "ccchccc", "ccccccc", "ccmcmcc", "ccc~ccc", "ccmcmcc", "ccccccc", "ccchccc" },
                    ...
                    .addElement(
                            ...
                            lazy(
                                    t -> buildHatchAdder(MTELargerTurbineBase.class)
                                            .adder(MTELargerTurbineBase::addTurbineHatch)
                                            .hatchClass(MTEHatchTurbine.class).casingIndex(t.getCasingTextureIndex())
                                            .dot(1).build()))
                    .addElement(
                            'h',
                            lazy(
    ...
    protected final MultiblockTooltipBuilder createTooltip() {
        ...
        tt.addPollutionAmount(getPollutionPerSecond(null)).beginStructureBlock(7, 9, 7, false)
                .addController("Top Middle").addCasingInfoMin(getCasingName(), 360, false)
                .addCasingInfoMin("Turbine Shaft", 30, false)
                .addOtherStructurePart(
                        StatCollector.translateToLocal("GTPP.tooltip.structure.rotor_assembly"),
                        "Any 1 dot hint",
                        1)
                .addInputBus("Any 4 dot hint (min 1)", 4).addInputHatch("Any 4 dot hint(min 1)", 4);
            ...
            if (!tFluids.isEmpty()) {
                if (baseEff == 0 || optFlow == 0
                    ...
                    aTotalOptimalFlow += GTUtility.safeInt(
                            (long) Math.max(Float.MIN_NORMAL, getSpeedMultiplier() * turbine.getOptimalFlow()));
    ...
    public String[] getExtraInfoData() {
        ...
        for (MTEHatchTurbine aHatch : this.getFullTurbineAssemblies()) {
            ...
            aTurbineDamage.append(EnumChatFormatting.RED).append(GTUtility.formatNumbers(tDura))
    ...
    protected ITexture getFrontFacingTurbineTexture(boolean isActive) {
        if (isActive) {
            return TextureFactory.builder().addIcon(TexturesGtBlock.Overlay_Machine_Controller_Advanced_Active)
                    .extFacing().build();
        }
        return TextureFactory.builder().addIcon(TexturesGtBlock.Overlay_Machine_Controller_Advanced).extFacing()
                .build();
    }
