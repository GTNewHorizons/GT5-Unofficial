...
public class MTENuclearSaltProcessingPlant extends GTPPMultiBlockBase<MTENuclearSaltProcessingPlant>
    ...
    protected MultiblockTooltipBuilder createTooltip() {
        ...
        tt.addMachineType(getMachineType()).addInfo("Processes depleted nuclear salts that come from the LFTR")
                ...
                .addMaintenanceHatch("Back Side, Opposite of Controller", 6).toolTipFinisher();
        return tt;
    }
            ...
            STRUCTURE_DEFINITION = StructureDefinition.<MTENuclearSaltProcessingPlant>builder()
                    ...
                    .addElement(
                            ...
                            buildHatchAdder(MTENuclearSaltProcessingPlant.class).atLeast(InputBus, InputHatch)
                                    .casingIndex(TAE.getIndexFromPage(0, 10)).dot(2).buildAndChain(
                                            onElementPass(
                                                    x -> ++x.casing,
                    ...
                    .addElement(
                            ...
                            buildHatchAdder(MTENuclearSaltProcessingPlant.class).atLeast(OutputBus, OutputHatch)
                                    .casingIndex(TAE.getIndexFromPage(0, 10)).dot(3).buildAndChain(
                                            onElementPass(
                                                    x -> ++x.casing,
                                                    ofBlock(ModBlocks.blockSpecialMultiCasings, 8))))
                    .addElement(
                    ...
                    .addElement(
                            ...
                            buildHatchAdder(MTENuclearSaltProcessingPlant.class).atLeast(Maintenance)
                                    .casingIndex(TAE.getIndexFromPage(0, 10)).dot(6).buildAndChain(
                                            onElementPass(
                                                    x -> ++x.casing,
                                                    ofBlock(ModBlocks.blockSpecialMultiCasings, 8))))
                    .build();
    ...
    protected ProcessingLogic createProcessingLogic() {
        return new ProcessingLogic().setSpeedBonus(1F / 2.5F).setMaxParallelSupplier(this::getTrueParallel);
    }
