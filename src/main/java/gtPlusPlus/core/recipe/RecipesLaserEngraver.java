...
public class RecipesLaserEngraver implements IOreRecipeRegistrator {
    ...
    public void registerOre(final OrePrefixes aPrefix, final Materials aMaterial, final String aOreDictName,
        ...
        if (aOreDictName.equals(OreDictNames.craftingLensWhite.toString())) {
            ...
            // T1
            GTValues.RA.stdBuilder().itemInputs(wireT1a, GTUtility.copyAmount(0L, aStack)).itemOutputs(coilWire1)
                    .duration(10 * SECONDS).eut(TierEU.RECIPE_HV).addTo(laserEngraverRecipes);
            GTValues.RA.stdBuilder().itemInputs(wireT1b, GTUtility.copyAmount(0L, aStack)).itemOutputs(coilWire1)
                    .duration(10 * SECONDS).eut(TierEU.RECIPE_HV).addTo(laserEngraverRecipes);
            // T2
            GTValues.RA.stdBuilder().itemInputs(wireT2a, GTUtility.copyAmount(0L, aStack)).itemOutputs(coilWire2)
                    .duration(20 * SECONDS).eut(TierEU.RECIPE_EV).addTo(laserEngraverRecipes);
            GTValues.RA.stdBuilder().itemInputs(wireT2b, GTUtility.copyAmount(0L, aStack)).itemOutputs(coilWire2)
                    .duration(20 * SECONDS).eut(TierEU.RECIPE_EV).addTo(laserEngraverRecipes);
            // T3
            GTValues.RA.stdBuilder().itemInputs(wireT3a, GTUtility.copyAmount(0L, aStack)).itemOutputs(coilWire3)
                    .duration(30 * SECONDS).eut(TierEU.RECIPE_IV).addTo(laserEngraverRecipes);
            GTValues.RA.stdBuilder().itemInputs(wireT3b, GTUtility.copyAmount(0L, aStack)).itemOutputs(coilWire3)
                    .duration(30 * SECONDS).eut(TierEU.RECIPE_IV).addTo(laserEngraverRecipes);
            GTValues.RA.stdBuilder().itemInputs(wireT3c, GTUtility.copyAmount(0L, aStack)).itemOutputs(coilWire3)
                    .duration(30 * SECONDS).eut(TierEU.RECIPE_IV).addTo(laserEngraverRecipes);
            // T4
            GTValues.RA.stdBuilder().itemInputs(wireT4a, GTUtility.copyAmount(0L, aStack)).itemOutputs(coilWire4)
            ...
            GTValues.RA.stdBuilder().itemInputs(GregtechItemList.AlkalusDisk.get(1), GTUtility.copyAmount(0L, aStack))
                    .itemOutputs(new ItemStack(DimensionEverglades.portalItem)).duration(3 * HOURS)
