...
public class FluidUtils {
    ...
    public static Fluid addGTPlasma(final Material aMaterial) {
        if (aMaterial.getLocalDefaultName()
            .toLowerCase()
            .contains("clay")
            || (aMaterial.getComposites()
                .size() > 1)
            || aMaterial.getLocalDefaultName()
                .toLowerCase()
                .contains("wrought")) {
            return null;
        }
        Logger.INFO("Generating a " + aMaterial.getLocalDefaultName() + " Plasma Cell");
        ...
        if (temp != null) {
            return addGTFluid(
                ...
                aMaterial.getLocalDefaultName() + " Plasma",
                aMaterial.getRGBA(),
                3,
                10000,
                temp,
