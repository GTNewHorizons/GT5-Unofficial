...
public class GT_MetaTileEntity_Hatch_Rack extends GT_MetaTileEntity_Hatch implements IAddGregtechLogo, IAddUIWidgets {
    ...
    private int getComputationPower(float overclock, float overvolt, boolean tickingComponents) {
        ...
        for (int i = 0; i < mInventory.length; i++) {
            ...
            RackComponent comp = componentBinds.get(TT_Utility.getUniqueIdentifier(mInventory[i]));
            if (comp == null) {
                continue;
            }
    ...
    public static class RackComponent implements Comparable<RackComponent> {
        ...
        RackComponent(ItemStack is, float computation, float heatConstant, float coolConstant, float maxHeat,
            boolean subZero) {
            this(TT_Utility.getUniqueIdentifier(is), computation, heatConstant, coolConstant, maxHeat, subZero);
        }
