    ...
    private static ArrayList<Pair<Materials, Long>> processDimension(
        ...
        if (normalOreDimWrapper != null) {
            normalOreDimWrapper.oreVeinToProbabilityInDimension.forEach((veinInfo, probability) -> {
                if (veinInfo.mPrimaryVeinMaterial instanceof Materials gtMat) processHelper(outputMap, gtMat, mainMultiplier, probability);
                if (veinInfo.mSecondaryMaterial instanceof Materials gtMat) processHelper(outputMap, gtMat, mainMultiplier, probability);
        ...
        if (smallOreDimWrapper != null) {
            smallOreDimWrapper.oreVeinProbabilities.forEach((veinInfo, probability) -> {
                if (veinInfo.material instanceof Materials gtMat) processHelper(outputMap, gtMat, mainMultiplier, probability);
            });
        }
