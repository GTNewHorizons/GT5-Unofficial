...
public class EyeOfHarmonyRecipeStorage {
    ...
    public EyeOfHarmonyRecipeStorage() {
        ...
        for (EyeOfHarmonyRecipe recipe : recipeHashMap.values()) {
            ...
            GTValues.RA.stdBuilder().itemInputs(planetItem).itemOutputs(outputItems.toArray(new ItemStack[0]))
                    .fluidInputs(
                            Materials.Hydrogen.getGas(0),
                            Materials.Helium.getGas(0),
                            MaterialsUEVplus.RawStarMatter.getFluid(0))
                    .fluidOutputs(outputFluids.toArray(new FluidStack[0])).duration(recipe.getRecipeTimeInTicks())
                    .eut(0).special(recipe).addTo(eyeOfHarmonyRecipes);
        }
    }
