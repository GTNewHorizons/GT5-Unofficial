...
/**
 * The Stabilization Matrix's structure definition as plain shape data (kept free of MTE references so the
 ...
 * the same transpose convention as the other Voidcraft structures. {@code A} = the plain casing block,
 * {@code ~} = the controller anchor (the machine block itself, bottom-center).
 *
 * <p>
 * 7 wide × 10 tall × 7 deep: 490 cells = 1 controller + 489 casings. A STATION-ONLY structure (station
 * infrastructure like the Satellite Rail Launcher — digitized into a Voidbase blueprint, rejected from a ship
 * build). The 7×7×10 footprint fits the Voidbase Assembler's 15×15×15 scan volume.
 */
public final class StabilizationMatrixStructure {
    ...
    /** Total structure cell count (every shape character, anchor included): 1 controller + 489 casings. */
    ...
    private StabilizationMatrixStructure() {}
    ...
    /**
     * @param element the element character to count ('A' or '~')
