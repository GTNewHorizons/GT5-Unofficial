 ...
 * constructor carrying a Voidbase blueprint (the parts it carries are its cargo) reaches its target and builds
 * or fills the construction site there (the first constructor creates it, later ones fill it; a completed site
 * spawns the base). TIMED: begin arms the site's part-transfer leg (one part per second per 100 construction
 * power) and the command runs RUNNING - the executor polls {@link #tick} every machine tick until the leg
 * completes, the site fills, or the leg counts down (leftover parts then stay on board). Unresolvable hover / no
 * blueprint / an occupied anchor -&gt; FAILED -&gt; the executor SKIPS the instruction.
 *
 * <p>
