...
public class MTELargeEssentiaSmeltery extends MTETooltipMultiBlockBaseEM
    ...
    public IStructureDefinition<? extends TTMultiblockBase> getStructure_EM() {
        if (this.multiDefinition == null) {
            this.multiDefinition = StructureDefinition.<MTELargeEssentiaSmeltery>builder()
                    ...
                    .addElement(
                            'F',
                            ThaumicBases.isModLoaded() ? ofBlock(
                                    Objects.requireNonNull(Block.getBlockFromName("thaumicbases:advAlchFurnace")),
                                    0) : ofBlock(ConfigBlocks.blockStoneDevice, 0))
                    .addElement(
                            'E',
                            ofChain(
                    ...
                    .addElement(
                            ...
                            ofChain(
                                    ...
                                    ofSpecificTileAdder(
                                            ...
                                            0),
                                    onElementPass(
                                            MTELargeEssentiaSmeltery::onCasingFound,
                                            ofBlock(Loaders.magicCasing, 0))))
                    .addElement('B', gregtech.api.enums.HatchElement.Muffler.newAny(CASING_INDEX, 2)).build();
        }
    ...
    protected MultiblockTooltipBuilder createTooltip() {
        ...
        tt.addMachineType("Essentia Smeltery, LES").addInfo("Necessary evil.")
                .addInfo("Advanced Essentia smelting technology.").addInfo("Maximum parallel = 2^Tier * (Length - 1)")
                .addInfo("Diffusion Cell Tiers start from 0, Length is full multi length.")
                .addInfo("Energy Hatch tier: HV+")
                .addInfo("You can find more information about this machine in the Thaumonomicon.").addTecTechHatchInfo()
                .addPollutionAmount(getPollutionPerSecond(null)).addController("Front center")
                .addCasingInfoMin("Magic Casing", 24, false).addMaintenanceHatch("Hint block with dot 1")
                .addInputBus("Hint block with dot 1").addInputHatch("Hint block with dot 1")
                .addEnergyHatch("Hint block with dot 1")
                .addOtherStructurePart("Essentia Output Hatch", "Hint block with dot 1")
                .addMufflerHatch("Hint block with dot 2").toolTipFinisher();
        return tt;
    }
    ...
    public @NotNull CheckRecipeResult checkProcessing_EM() {
        ...
        OverclockCalculator calculator = new OverclockCalculator().setRecipeEUt(RECIPE_EUT).setEUt(getMaxInputEu())
                .setDuration(
                        (int) Math.ceil(
                                this.mOutputAspects.visSize() * RECIPE_DURATION * (1 - this.nodeIncrease * 0.005)))
