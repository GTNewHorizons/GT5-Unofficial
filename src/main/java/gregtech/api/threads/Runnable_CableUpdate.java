...
public class Runnable_CableUpdate extends Runnable_MachineBlockUpdate {

    protected Runnable_CableUpdate(World aWorld, int posX, int posY, int posZ) {
        super(aWorld, posX, posY, posZ);
    }

    public static void setCableUpdateValues(World aWorld, int posX, int posY, int posZ) {
        if (isEnabled) {
            EXECUTOR_SERVICE.submit(new Runnable_CableUpdate(aWorld, posX, posY, posZ));
        }
    }
        ...
        try {
            while (!tQueue.isEmpty()) {
                ...
                if (tTileEntity instanceof BaseMetaPipeEntity metaPipe
                    && metaPipe.getMetaTileEntity() instanceof MetaPipeEntity_Cable cable) {
