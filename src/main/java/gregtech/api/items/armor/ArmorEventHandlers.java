...
public class ArmorEventHandlers {
    ...
    public void onLivingEquipmentChange(LivingEquipmentChangeEvent event) {
        ...
        if (from != null) {
            ...
            if (to != null && event.getFrom().isItemEqual(event.getTo())) {
                return;
            }

            if (event.getFrom().getItem() instanceof MechArmorBase armor) {
                armor.onArmorUnequip(player.getEntityWorld(), player, event.getFrom());
            }
        }
        if (to != null) {
            if (event.getTo().getItem() instanceof MechArmorBase armor) {
