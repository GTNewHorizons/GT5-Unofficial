        ...
        if (aID >= 0 && aID < mItemAmount) {
            ...
            for (Object tRandomData : aRandomData) if (tRandomData != null) {
                ...
                if (tRandomData instanceof ItemData) {
                    if (GT_Utility.isStringValid(tRandomData)) OreDictUnificator.registerOre(tRandomData, rStack);
                    else OreDictUnificator.addItemData(rStack, (ItemData) tRandomData);
                    continue;
                }
                if (tUseOreDict) {
