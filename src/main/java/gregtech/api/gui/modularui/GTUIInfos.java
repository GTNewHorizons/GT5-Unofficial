    ...
    static {
        for (final ForgeDirection side : ForgeDirection.VALID_DIRECTIONS) {
            coverUI.put(side, UIBuilder.of().container((player, world, x, y, z) -> {
                ...
                return gtTileEntity.getCoverAtSide(side).createCoverContainer(player);
            }).gui((player, world, x, y, z) -> {
                if (!world.isRemote) return null;
                final TileEntity te = world.getTileEntity(x, y, z);
                if (!(te instanceof ICoverable gtTileEntity)) return null;
                ModularUIContainer container = gtTileEntity.getCoverAtSide(side).createCoverContainer(player);
