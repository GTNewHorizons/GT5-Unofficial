...
public interface IExperimentalEnergyTileEntity extends IColoredTileEntity, IHasWorldObjectAndCoords {
    ...
    class Util {
        ...
        public static final long emitEnergyToNetwork(SubTag aEnergyType, long aPrimary, long aSecondary, IExperimentalEnergyTileEntity aEmitter) {
            ...
            checkAvailabilities();
            for (int i = 0, j = 0; i < 6 && aSecondary > rUsedSecondary; i++)
                if (aEmitter.outputsEnergyTo(aEnergyType, (byte)i)) {
                    j = GT_Utility.getOppositeSide(i);
                    TileEntity tTileEntity = aEmitter.getTileEntityAtSide((byte)i);
                    if (tTileEntity instanceof IExperimentalEnergyTileEntity) {
                        ...
                        rUsedSecondary += ((IExperimentalEnergyTileEntity) tTileEntity).injectEnergy(aEnergyType, (byte)j, aPrimary, aSecondary - rUsedSecondary);
