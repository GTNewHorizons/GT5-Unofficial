...
public interface IItemBehaviour<E extends Item> {
     ...
     * your item's right click activation in {@link #onItemUse(Item, ItemStack, EntityPlayer, World, int, int, int, int, float, float, float) onItemUse}
     * for best results.
     * <p>
     * Typically used when the item needs support for the Ring of Loki (from Botania.) If you don't care about that,
     * using {@link #onItemUseFirst(Item, ItemStack, EntityPlayer, World, int, int, int, ForgeDirection, float, float, float) onItemUseFirst}
     * instead of {@link #onItemUse(Item, ItemStack, EntityPlayer, World, int, int, int, int, float, float, float) onItemUse}
