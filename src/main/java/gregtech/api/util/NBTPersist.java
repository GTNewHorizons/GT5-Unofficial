...
public class NBTPersist {
    ...
    public static JsonElement toJsonObject(NBTBase nbt) {
        switch (nbt) {
            ...
            case NBTTagByte nbtTagByte -> {
                // Number (byte)
                return new JsonPrimitive(nbtTagByte.func_150290_f());
                // Number (byte)
            }
            case NBTTagShort nbtTagShort -> {
                // Number (short)
                return new JsonPrimitive(nbtTagShort.func_150289_e());
                // Number (short)
            }
            case NBTTagInt nbtTagInt -> {
                // Number (int)
                return new JsonPrimitive(nbtTagInt.func_150287_d());
                // Number (int)
            }
            case NBTTagLong nbtTagLong -> {
                // Number (long)
                return new JsonPrimitive(nbtTagLong.func_150291_c());
                // Number (long)
            }
            case NBTTagFloat nbtTagFloat -> {
                // Number (float)
                return new JsonPrimitive(nbtTagFloat.func_150288_h());
                // Number (float)
            }
            case NBTTagDouble nbtTagDouble -> {
                // Number (double)
                return new JsonPrimitive(nbtTagDouble.func_150286_g());
                // Number (double)
            }
            case NBTBase.NBTPrimitive nbtPrimitive -> {
                // Number
                return new JsonPrimitive(nbtPrimitive.func_150286_g());
                // Number
            }
    ...
    public static JsonElement toJsonObjectExact(NBTBase nbt) {
        switch (nbt) {
            ...
            case NBTBase.NBTPrimitive other -> {
