...
public class GT_RecipeConstants {
    ...
    public static final IGT_RecipeMap UniversalChemical = builder -> {
        for (ItemStack input : builder.getItemInputsBasic()) {
            if (GT_Utility.isAnyIntegratedCircuit(input) && input.getItemDamage() >= 10) return GT_Utility.concat(
                builder.copy().add(GT_Recipe_Map.sChemicalRecipes),
                // LCR does not need cleanroom, for now.
                builder.metadata(CLEANROOM, false).add(GT_Recipe_Map.sMultiblockChemicalRecipes));
        }
        return builder.add(GT_Recipe_Map.sChemicalRecipes);
    };

    public static final IGT_RecipeMap AssemblyLine = builder -> {
        Optional<GT_Recipe.GT_Recipe_WithAlt> rr = builder.validateInputCount(4, 16)
        ...
        ret.add(GT_Recipe.GT_Recipe_Map.sScannerFakeRecipes.addFakeRecipe(
            false,
        ...
        ret.add(GT_Recipe.GT_Recipe_Map.sAssemblylineVisualRecipes.addFakeRecipe(
            false,
            r.mInputs,
            new ItemStack[] { aOutput },
    ...
    public static IGT_RecipeMap AssemblerOD = builder -> {
        ...
        for (ItemStack input : GT_OreDictUnificator.getOresImmutable(builder.getMetadata(OREDICT_INPUT))) {
            ret.addAll(builder.copy().itemInputs(GT_RecipeMapUtil.appendArray(builder.getItemInputsBasic(), input)).add(GT_Recipe_Map.sAssemblerRecipes));
        }
        return ret;
    };
    ...
    public static IGT_RecipeMap Fuel = builder -> {
        ...
        builder.specialValue(GregTech_API.sRecipeFile.get("fuel_" + fuelType, builder.getItemInputBasic(0), builder.getMetadata(FUEL_VALUE)));
