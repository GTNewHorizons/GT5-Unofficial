package gregtech.api.util;

public class CustomGlyphs {

    public static final String AIR = "\uE000";
    public static final String EARTH = "\uE001";
    public static final String FIRE = "\uE002";
    public static final String WATER = "\uE003";
    public static final String BRIMSTONE = "\uE004";
    public static final String CIRCLE_CROSS = "\uE005";
    public static final String CIRCLE_STAR = "\uE006";
    public static final String PICKAXE = "\uE007";
    public static final String ARROW_CORNER_SOUTH_EAST = "\uE008";
    public static final String ARROW_CORNER_NORTH_WEST = "\uE009";
    public static final String HIGH_VOLTAGE = "\uE00A";
    public static final String SUPERSCRIPT0 = "\uE010";
    public static final String SUPERSCRIPT1 = "\uE011";
    public static final String SUPERSCRIPT2 = "\uE012";
    public static final String SUPERSCRIPT3 = "\uE013";
    public static final String SUPERSCRIPT4 = "\uE014";
    public static final String SUPERSCRIPT5 = "\uE015";
    public static final String SUPERSCRIPT6 = "\uE016";
    public static final String SUPERSCRIPT7 = "\uE017";
    public static final String SUPERSCRIPT8 = "\uE018";
    public static final String SUPERSCRIPT9 = "\uE019";
    public static final String SUBSCRIPT0 = "\uE01A";
}
