...
public enum ApiaryUpgrade {
    ...
    private enum UNIQUE_INDEX {
        ...
        void apply(Consumer<ApiaryUpgrade> fn) {
            UNIQUE_UPGRADE_LIST.get(this)
                .forEach(fn);
        }
    }

    private static final EnumMap<UNIQUE_INDEX, ArrayList<ApiaryUpgrade>> UNIQUE_UPGRADE_LIST = new EnumMap<>(
        UNIQUE_INDEX.class);
    ...
    private final BiConsumer<ApiaryModifier, Integer> applier;

    private final HashSet<GT_Utility.ItemId> blacklistedUpgrades = new HashSet<>();

    ApiaryUpgrade(UNIQUE_INDEX unique_index, int meta, int maxnumber, BiConsumer<ApiaryModifier, Integer> applier) {
    ...
    private void setup_static_variables() {
        ...
        ArrayList<ApiaryUpgrade> un = UNIQUE_UPGRADE_LIST.get(this.unique_index);
    ...
    public static ApiaryUpgrade getUpgrade(ItemStack s) {
    ...
    public void applyModifiers(ApiaryModifier mods, ItemStack stack) {
    ...
    private static final HashMap<Integer, ApiaryUpgrade> quickLookup = new HashMap<>();

    static {
        EnumSet.allOf(ApiaryUpgrade.class)
            .forEach(ApiaryUpgrade::setup_static_variables);
