...
/// group flag without every shape in that group actually generating (removed via a `REMOVED_PREFIXES` entry that
/// `LegacyMaterials#build` adds to `OrePrefixes#mNotGeneratedItems`), and a handful of consumers
/// (recipe/block/pipe loaders) read the group flag itself rather than checking for a specific shape's existence.
