    ...
    protected GTRecipe overwriteFindRecipe(ItemStack[] items, FluidStack[] fluids, @Nullable ItemStack specialSlot,
        ...
        if (GTUtility.areStacksEqual(items[0], new ItemStack(Items.book, 1, WILDCARD))) {
            return GTValues.RA.stdBuilder().itemInputs(GTUtility.copyAmount(1, items[0]))
                    ...
                    .duration(32).eut(4).build().orElse(null);
        }
        ...
        for (ItemStack item : new ItemStack[] { GTUtility.getContainerItem(items[0], true), items[0],
            ...
            if (GTUtility.areStacksEqual(item, new ItemStack(Blocks.netherrack, 1, WILDCARD), true)
                ...
                return GTRecipeBuilder.empty().metadata(EXPLODE, true).build().orElse(null);
            }
            ...
            if (itemData != null) {
                if (itemData.mMaterial != null && itemData.mMaterial.mMaterial != null) {
                    ...
                    if (itemData.mMaterial.mMaterial.contains(SubTag.FLAMMABLE)) {
                        ...
                        return GTRecipeBuilder.empty().metadata(ON_FIRE, true).build().orElse(null);
                    }
                }
