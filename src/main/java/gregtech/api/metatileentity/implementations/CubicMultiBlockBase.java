 ...
 * @param <T>
 */
public abstract class CubicMultiBlockBase<T extends CubicMultiBlockBase<T>> extends EnhancedMultiBlockBase<T>
    implements ISurvivalConstructable {

    protected static final String STRUCTURE_PIECE_MAIN = "main";
    protected static final ClassValue<IStructureDefinition<CubicMultiBlockBase<?>>> STRUCTURE_DEFINITION = new ClassValue<>() {

        @Override
        protected IStructureDefinition<CubicMultiBlockBase<?>> computeValue(Class<?> type) {
            return StructureDefinition.<CubicMultiBlockBase<?>>builder()
                .addShape(
                    ...
                    ofChain(
                        lazy(
                            t -> StructureUtility.<CubicMultiBlockBase<?>>buildHatchAdder()
                                ...
                                .build()),
                        onElementPass(
                            CubicMultiBlockBase::onCorrectCasingAdded,
                            lazy(CubicMultiBlockBase::getCasingElement))))
                .build();
        }
    };
    private int mCasingAmount = 0;

    protected CubicMultiBlockBase(int aID, String aName, String aNameRegional) {
        super(aID, aName, aNameRegional);
    }

    protected CubicMultiBlockBase(String aName) {
        super(aName);
    }
    ...
    protected abstract IStructureElement<CubicMultiBlockBase<?>> getCasingElement();

    protected List<IHatchElement<? super CubicMultiBlockBase<?>>> getAllowedHatches() {
