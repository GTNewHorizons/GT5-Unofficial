...
public class Hatch_Maintenance extends Hatch implements IAddUIWidgets, IAlignment {
    ...
    public Hatch_Maintenance(int aID, String aName, String aNameRegional, int aTier) {
        super(aID, aName, aNameRegional, aTier, 1, "For maintaining Multiblocks");
        mAuto = false;
    }

    public Hatch_Maintenance(int aID, String aName, String aNameRegional, int aTier, boolean aAuto) {
        super(aID, aName, aNameRegional, aTier, 4, "For automatically maintaining Multiblocks");
        mAuto = aAuto;
    }

    public Hatch_Maintenance(String aName, int aTier, String aDescription, ITexture[][][] aTextures, boolean aAuto) {
    ...
    private static ItemStack[] getAutoMaintenanceInputs() {
        if (sAutoMaintenanceInputs == null) sAutoMaintenanceInputs = new ItemStack[] { ItemList.Duct_Tape.get(4),
            OreDictUnificator.get(OrePrefixes.cell, Materials.Lubricant, 2),
    ...
    public MetaTileEntity newMetaEntity(IGregTechTileEntity aTileEntity) {
        if (aTileEntity.getMetaTileID() == 111)
            return new Hatch_Maintenance(mName, mTier, mDescriptionArray, mTextures, true);
        return new Hatch_Maintenance(mName, mTier, mDescriptionArray, mTextures, false);
    }
    ...
    public boolean onRightclick(IGregTechTileEntity aBaseMetaTileEntity, EntityPlayer aPlayer, ForgeDirection side,
        ...
        if (side == aBaseMetaTileEntity.getFrontFacing()) {
            ...
            if (tStack != null) {
                ...
                } else UIInfos.openGTTileEntityUI(aBaseMetaTileEntity, aPlayer);
            } else {
            ...
            if (tStack != null) {
                ...
                for (ItemStack aStack : mInventory) {
                    if ((GT_Utility.areUnificationsEqual(aStack, tStack, true)
                        || GT_Utility.areUnificationsEqual(OreDictUnificator.get(false, aStack), tStack, true))) {
    ...
    public void onToolClick(ItemStack aStack, EntityLivingBase aPlayer, IInventory aToolboxInventory) {
        ...
        if (!mSolderingTool && GT_ModHandler.useSolderingIron(aStack, aPlayer, aToolboxInventory))
            mSolderingTool = true;
        if (OreDictUnificator.isItemStackInstanceOf(aStack, "craftingDuctTape")) {
            mWrench = mScrewdriver = mSoftHammer = mHardHammer = mCrowbar = mSolderingTool = true;
            getBaseMetaTileEntity().setActive(false);
            aStack.stackSize--;
        }
    ...
    public boolean allowPutStack(IGregTechTileEntity aBaseMetaTileEntity, int aIndex, ForgeDirection side,
        ...
        if (mAuto && GT_Mod.gregtechproxy.mAMHInteraction) {
            for (int i = 0; i < getSizeInventory(); i++) if (GT_Utility
                .areStacksEqual(OreDictUnificator.get(false, aStack), OreDictUnificator.get(false, getStackInSlot(i))))
                return i == aIndex;
            for (ItemStack tInput : getAutoMaintenanceInputs())
                if (GT_Utility.areUnificationsEqual(tInput, aStack, true)
                    || GT_Utility.areUnificationsEqual(OreDictUnificator.get(false, aStack), tInput, true)) return true;
        }
        return false;
    }
    ...
    public void addUIWidgets(ModularWindow.Builder builder, UIBuildContext buildContext) {
        if (mAuto) {
            ...
            builder.widget(
                new DrawableWidget().setDrawable(UITextures.SLOT_MAINTENANCE)
                    .setPos(78, 33)
                    .setSize(20, 20))
