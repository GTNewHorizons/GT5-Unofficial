...
public class MTEMultiBlockBaseGui {
    ...
    public ModularPanel build(PosGuiData guiData, PanelSyncManager syncManager, UISettings uiSettings) {
        ...
        ModularPanel panel = new ModularPanel("MTEMultiBlockBase").size(198, 181 + textBoxToInventoryGap).padding(4);
        return panel.child(
                new Column().sizeRel(1).child(createTitleTextStyle(base.getLocalName()))
                        .child(createTerminalRow(panel, syncManager)).child(createPanelGap(panel, syncManager))
                        .child(createInventoryRow(panel, syncManager)));
    }

    private IWidget createTitleTextStyle(String title) {
        return new SingleChildWidget<>().coverChildren().topRel(0, -4, 1).leftRel(0, -4, 0)
                .widgetTheme(GTWidgetThemes.BACKGROUND_TITLE).child(
                        IKey.str(title).asWidget().alignment(Alignment.Center).widgetTheme(GTWidgetThemes.TEXT_TITLE)
                                .marginLeft(5).marginRight(5).marginTop(5).marginBottom(1));
    }

    protected Flow createTerminalRow(ModularPanel panel, PanelSyncManager syncManager) {
        return new Row().size(getTerminalRowWidth(), getTerminalRowHeight()).child(
                new ParentWidget<>().size(getTerminalWidgetWidth(), getTerminalWidgetHeight()).padding(4)
                        ...
                        .child(
                                new SingleChildWidget<>().bottomRel(0, 10, 0).rightRel(0, 10, 0).size(18, 18)
        ...
        return new ListWidget<>()
                .child(
                        new TextWidget(GTUtility.trans("132", "Pipe is loose. (Wrench)")).color(Color.WHITE.main)
                                .setEnabledIf(widget -> !base.mWrench).marginBottom(2).widthRel(1))
                .child(
                        new TextWidget(GTUtility.trans("133", "Screws are loose. (Screwdriver)"))
                                .color(Color.WHITE.main).setEnabledIf(widget -> !base.mScrewdriver).marginBottom(2)
                                .widthRel(1))
                .child(
                        new TextWidget(GTUtility.trans("134", "Something is stuck. (Soft Mallet)"))
                                .color(Color.WHITE.main).setEnabledIf(widget -> !base.mSoftMallet).marginBottom(2)
                                .widthRel(1))
                .child(
                        new TextWidget(GTUtility.trans("135", "Platings are dented. (Hammer)")).color(Color.WHITE.main)
                                .setEnabledIf(widget -> !base.mHardHammer).marginBottom(2).widthRel(1))
                .child(
                        new TextWidget(GTUtility.trans("136", "Circuitry burned out. (Soldering)"))
                                .color(Color.WHITE.main).setEnabledIf(widget -> !base.mSolderingTool).marginBottom(2)
                                .widthRel(1))
                .child(
                        new TextWidget(GTUtility.trans("137", "That doesn't belong there. (Crowbar)"))
                                .color(Color.WHITE.main).setEnabledIf(widget -> !base.mCrowbar).marginBottom(2)
                                .widthRel(1))
                .child(
                        new TextWidget(GTUtility.trans("138", "Incomplete Structure.")).color(Color.WHITE.main)
                                .setEnabledIf(widget -> !base.mMachine).marginBottom(2).widthRel(1))
                .child(
                        new TextWidget(StatCollector.translateToLocal("GT5U.gui.text.too_uncertain"))
                                .color(Color.WHITE.main).setEnabledIf(widget -> (base.getErrorDisplayID() & 128) != 0)
                                .marginBottom(2).widthRel(1))
                .child(
                        new TextWidget(StatCollector.translateToLocal("GT5U.gui.text.invalid_parameters"))
                                .color(Color.WHITE.main).setEnabledIf(widget -> (base.getErrorDisplayID() & 256) != 0)
                                .marginBottom(2).widthRel(1))
                ...
                .child(
                        new TextWidget(
                                IKey.comp(
                                        ...
                                        IKey.lang("gt.interact.desc.mb.idle.3")))
                ...
                .child(createShutdownDurationWidget()).child(createShutdownReasonWidget())
    ...
    private IWidget createShutdownDurationWidget() {
        return IKey.dynamic(() -> {
            ...
            return StatCollector.translateToLocalFormatted(
                    ...
                    time.toMinutes() % 60,
                    time.getSeconds() % 60);
        }).asWidget().marginBottom(2).widthRel(1).setEnabledIf(
    ...
    private IWidget createShutdownReasonWidget() {
        return IKey.dynamic(() -> baseMetaTileEntity.getLastShutDownReason().getDisplayString()).asWidget()
                .marginBottom(2).widthRel(1).setEnabledIf(widget -> shouldShutdownReasonBeDisplayed());
    }

    private boolean shouldShutdownReasonBeDisplayed() {
        return base.shouldDisplayShutDownReason() && !baseMetaTileEntity.isActive()
                && !baseMetaTileEntity.isAllowedToWork()
                && GTUtility.isStringValid(baseMetaTileEntity.getLastShutDownReason().getDisplayString());
    }

    private IWidget createRecipeResultWidget() {
        return IKey.dynamic(() -> base.getCheckRecipeResult().getDisplayString()).asWidget().marginBottom(2).widthRel(1)
    ...
    private IWidget createRecipeInfoWidget(PanelSyncManager syncManager) {
        return IKey.dynamic(() -> ((StringSyncValue) syncManager.getSyncHandler("recipeInfo:0")).getValue()).asWidget()
                .marginBottom(2).widthRel(1).setEnabledIf(
    ...
    protected IWidget createPanelGap(ModularPanel parent, PanelSyncManager syncManager) {
        return new Row().widthRel(1).paddingRight(6).paddingLeft(4).height(textBoxToInventoryGap)
                .child(createVoidExcessButton(syncManager)).child(createInputSeparationButton(syncManager))
                .childIf(!machineModeIcons.isEmpty(), createModeSwitchButton(syncManager))
                .child(createBatchModeButton(syncManager)).child(createLockToSingleRecipeButton(syncManager))
    ...
    private boolean voidExcessOnMousePressed(int mouseButton, IntSyncValue voidExcessSyncer) {
        ...
        int voidingMode = 0;
        switch (mouseButton) {
            case 0 -> voidingMode = base.getVoidingMode().nextInCollection(allowed).ordinal();
            case 1 -> voidingMode = base.getVoidingMode().previousInCollection(allowed).ordinal();
        }
    ...
    private void createVoidExcessTooltip(RichTooltip t) {
        t.addLine(IKey.dynamic(() -> StatCollector.translateToLocal("GT5U.gui.button.voiding_mode")))
                .addLine(IKey.dynamic(() -> StatCollector.translateToLocal(base.getVoidingMode().getTransKey())));
    ...
    protected IWidget createInputSeparationButton(PanelSyncManager syncManager) {
        ...
        }.size(18, 18).value(getInputSeparationSyncValue(inputSeparationSyncer))
                .overlay(
    ...
    protected IWidget createBatchModeButton(PanelSyncManager syncManager) {
        ...
        }.size(18, 18).value(getBatchModeSyncValue(batchModeSyncer))
    ...
    protected IWidget createLockToSingleRecipeButton(PanelSyncManager syncManager) {
        ...
        }.size(18, 18).value(getRecipeLockSyncValue(recipeLockSyncer))
                .overlay(
    ...
    protected IWidget createPowerPanelButton(PanelSyncManager syncManager, ModularPanel parent) {
        IPanelHandler powerPanel = syncManager.panel(
                ...
                true);
        return new ButtonWidget<>().size(18, 18).rightRel(0, 6, 0).marginTop(4)
                .overlay(UITexture.fullImage(GregTech.ID, "gui/overlay_button/power_panel")).onMousePressed(d -> {
    ...
    private ModularPanel openPowerControlPanel(PanelSyncManager syncManager, ModularPanel parent) {
        ...
        return new ModularPanel("powerPanel").pos(x, y).size(120, 130).child(
                new Column().sizeRel(1).padding(3).child(makeTitleTextWidget())
                        .child(IKey.lang("GTPP.CC.parallel").asWidget().marginBottom(4))
    ...
    private IWidget makeTitleTextWidget() {
        return new TextWidget(
                EnumChatFormatting.UNDERLINE + StatCollector.translateToLocal("GT5U.gui.text.power_panel"))
                        .alignment(Alignment.Center).size(120, 18).marginBottom(4);
    }
    ...
    private IWidget makeParallelConfigurator(PanelSyncManager syncManager) {
        ...
        return new Row().widthRel(1).height(18).paddingLeft(3).paddingRight(3).mainAxisAlignment(MainAxis.CENTER)
                .child(
                        new TextFieldWidget().value(powerPanelMaxParallelSyncer).setTextAlignment(Alignment.Center)
                                .setNumbers(
                                                ...
                                                IKey.dynamic(
                                                        () -> alwaysMaxParallelSyncer.getValue()
                                                                ...
                                                                : StatCollector.translateToLocalFormatted(
                                                                        ...
                                                                        1,
                                                                        maxParallelSyncer.getValue()))))
                                .tooltipShowUpTimer(TOOLTIP_DELAY).size(70, 14).marginBottom(4))
                .child(new ButtonWidget<>().size(18, 18).overlay(new DynamicDrawable(() -> {
    ...
    private IWidget createInventoryRow(ModularPanel panel, PanelSyncManager syncManager) {
        return new Row().widthRel(1).height(76).alignX(0)
                .childIf(base.doesBindPlayerInventory(), SlotGroupWidget.playerInventory(false).marginLeft(4))
                .child(createButtonColumn(panel, syncManager));
    }

    protected Flow createButtonColumn(ModularPanel panel, PanelSyncManager syncManager) {
        return new Column().width(18).leftRel(1, -2, 1).mainAxisAlignment(MainAxis.END)
                .child(createStructureUpdateButton(syncManager)).child(createPowerSwitchButton()).childIf(
                        base.doesBindPlayerInventory(),
                        new ItemSlot().slot(
                                new ModularSlot(base.inventoryHandler, base.getControllerSlotIndex())
                                        .slotGroup("item_inv")));
    }

    protected IWidget createStructureUpdateButton(PanelSyncManager syncManager) {
        return new ToggleButton().size(18, 18).syncHandler("structureUpdateButton")
    ...
    protected IWidget createPowerSwitchButton() {
        return new ToggleButton().syncHandler("powerSwitch").tooltip(tooltip -> tooltip.add("Power Switch"))
                .size(18, 18).marginBottom(4).overlay(
                        new DynamicDrawable(
    ...
    protected void registerSyncValues(PanelSyncManager syncManager) {
        ...
        syncManager.syncValue(
                "screwdriver",
                new BooleanSyncValue(() -> base.mScrewdriver, val -> base.mScrewdriver = val));
        syncManager
        ...
        syncManager.syncValue(
                ...
                new GenericListSyncHandler<FluidStack>(
                        () -> base.mOutputFluids != null ? Arrays.stream(base.mOutputFluids).map(fluidStack -> {
