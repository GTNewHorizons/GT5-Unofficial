 ...
 * Machine
 */
public abstract class BasicMachine extends BasicTank implements RecipeMapWorkable, IConfigurationCircuitSupport,
     ...
     */
    public BasicMachine(int aID, String aName, String aNameRegional, int aTier, int aAmperage, String aDescription,
     ...
     */
    public BasicMachine(String aName, int aTier, int aAmperage, String[] aDescription, ITexture[][][] aTextures,
                ...
                if (mMaxProgresstime <= 0 && aBaseMetaTileEntity.isAllowedToWork()
                    ...
                    if (checkRecipe() == FOUND_AND_SUCCESSFULLY_USED_RECIPE) {
                        ...
                        for (int i = 0; i < mOutputItems.length; i++) {
                            ...
                            mOutputItems[i] = OreDictUnificator.get(true, mOutputItems[i]);
                        }
    ...
    public boolean allowCoverOnSide(ForgeDirection side, GT_ItemStack aCoverID) {
        ...
        CoverBehaviorBase<?> tBehavior = GregTech_API.getCoverBehaviorNew(aCoverID.toStack());
    ...
    public boolean allowPutStack(IGregTechTileEntity aBaseMetaTileEntity, int aIndex, ForgeDirection side,
        ...
        for (int i = getInputSlot(), j = i + mInputSlotCount; i < j; i++)
            if (GT_Utility.areStacksEqual(OreDictUnificator.get(aStack), mInventory[i]) && mDisableMultiStack)
                return i == aIndex;
    ...
    public void addUIWidgets(ModularWindow.Builder builder, UIBuildContext buildContext) {
        ...
        builder.widget(
            createErrorStatusArea(
                builder,
                isSteampowered() ? UITextures.PICTURE_STALLED_STEAM : UITextures.PICTURE_STALLED_ELECTRICITY));
    }
    ...
    protected CycleButtonWidget createItemAutoOutputButton() {
        return (CycleButtonWidget) new CycleButtonWidget().setToggle(() -> mItemTransfer, val -> mItemTransfer = val)
            .setStaticTexture(UITextures.OVERLAY_BUTTON_AUTOOUTPUT_ITEM)
