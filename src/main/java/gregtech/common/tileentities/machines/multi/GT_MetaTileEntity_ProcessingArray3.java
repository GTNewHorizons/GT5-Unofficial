...
public class GT_MetaTileEntity_ProcessingArray3 extends GT_MetaTileEntity_MultiBlockBase {
    ...
    public String[] getDescription() {
        return new String[]{
                "Controller Block for the Processing Array T3",
                ...
                "Fusion Machine Casings for the rest (16 at least!)",
                "Place up to 64 Single Block GT Machines into the Controller Inventory"};
    }

    public ITexture[] getTexture(IGregTechTileEntity aBaseMetaTileEntity, byte aSide, byte aFacing, byte aColorIndex, boolean aActive, boolean aRedstone) {
        if (aSide == aFacing) {
            return new ITexture[]{Textures.BlockIcons.CASING_BLOCKS[54], new GT_RenderedTexture(aActive ? Textures.BlockIcons.OVERLAY_FRONT_PROCESSING_ARRAY_ACTIVE : Textures.BlockIcons.OVERLAY_FRONT_PROCESSING_ARRAY)};
        }
        return new ITexture[]{Textures.BlockIcons.CASING_BLOCKS[54]};
    }
            ...
            for (int j = -1; j < 2; j++) {
                for (int h = -1; h < 2; h++) {
                    if ((h != 0) || (((xDir + i != 0) || (zDir + j != 0)) && ((i != 0) || (j != 0)))) {
                        ...
                        if ((!addMaintenanceToMachineList(tTileEntity, 54)) && (!addInputToMachineList(tTileEntity, 54)) && (!addOutputToMachineList(tTileEntity, 54)) && (!addEnergyInputToMachineList(tTileEntity, 54))) {
