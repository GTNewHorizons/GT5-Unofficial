    ...
    public void addUIWidgets(CoverGuiData guiData, PanelSyncManager syncManager, Flow column) {
        ...
        column.child(
            new Grid().marginLeft(WIDGET_MARGIN)
                ...
                .row(
                    new ToggleButton().value(LinkedBoolValue.of(modeSyncValue, ShutterMode.OPEN_IF_ENABLED))
                        .overlay(true, GTGuiTextures.OVERLAY_BUTTON_CHECKMARK)
                        .size(16),
                    IKey.str(GTUtility.trans("082", "Open if work enabled"))
                        .asWidget())
                .row(
                    new ToggleButton().value(LinkedBoolValue.of(modeSyncValue, ShutterMode.OPEN_IF_DISABLED))
                        .overlay(true, GTGuiTextures.OVERLAY_BUTTON_CHECKMARK)
                        .size(16),
                    IKey.str(GTUtility.trans("083", "Open if work disabled"))
                        .asWidget())
                .row(
                    new ToggleButton().value(LinkedBoolValue.of(modeSyncValue, ShutterMode.ONLY_OUTPUT))
                        .overlay(true, GTGuiTextures.OVERLAY_BUTTON_CHECKMARK)
                        .size(16),
                    IKey.str(GTUtility.trans("084", "Only Output allowed"))
                        .asWidget())
                .row(
                    new ToggleButton().value(LinkedBoolValue.of(modeSyncValue, ShutterMode.ONLY_INPUT))
                        .overlay(true, GTGuiTextures.OVERLAY_BUTTON_CHECKMARK)
                        .size(16),
