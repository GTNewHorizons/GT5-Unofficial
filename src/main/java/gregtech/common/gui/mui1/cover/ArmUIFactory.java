        ...
        builder.widget(
                new CoverDataControllerWidget<>(this::getCover, getUIBuildContext()).addFollower(
                        ...
                        (cover, state) -> {
                            if (state) {
                                return cover.setVariable(
                                        cover.getVariable() | CoverArm.EXPORT_MASK | CoverArm.CONVERTED_BIT);
                            } else {
                                return cover.setVariable(
                                        cover.getVariable() & ~CoverArm.EXPORT_MASK | CoverArm.CONVERTED_BIT);
                            }
                        ...
                        widget -> widget.setStaticTexture(GTUITextures.OVERLAY_BUTTON_EXPORT)
                                .addTooltip(translateToLocal("gt.interact.desc.export")).setPos(spaceX * 0, spaceY * 0))
                        .addFollower(
                                CoverDataFollowerToggleButtonWidget.ofDisableable(),
                                cover -> getFlagExport(cover.getVariable()) == 0,
                                (cover, state) -> {
                        ...
                        .addFollower(
                                ...
                                (cover, state) -> {
                                    ...
                                    return cover.setVariable(
                                            getFlagExport(coverVariable)
                                                    | ((state.intValue() + 1) & CoverArm.SLOT_ID_MASK)
                                ...
                                widget -> widget.setBounds(-1, maxSlot).setDefaultValue(-1).setScrollValues(1, 100, 10)
                                        .setNumberFormat(numberFormatAny).setPos(spaceX * 0, spaceY * 1 + 2)
                        ...
                        .addFollower(
                                ...
                                }).setMinValue(-1).setDefaultValue(-1).setScrollValues(1, 100, 10)
                                        .setNumberFormat(numberFormatAny).setPos(spaceX * 0, spaceY * 2 + 2)
                                        .setSize(spaceX * 2 + 5, 12))
                ...
                .widget(
                        new TextWidget(GTUtility.trans("254.1", "Internal slot#"))
                                .setDefaultColor(COLOR_TEXT_GRAY.get())
