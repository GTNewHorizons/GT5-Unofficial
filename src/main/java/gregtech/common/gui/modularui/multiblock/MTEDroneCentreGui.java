...
public class MTEDroneCentreGui extends MTEMultiBlockBaseGui<MTEDroneCentre> {
        ...
        droneConnectionListSyncHandler.setChangeListener(() -> {
            if (dynamicWidget.hasChildren()) {
                ...
                if (child instanceof ListWidget list) {
                    this.lastScrollY = list.getScrollY();
                }
            }
            droneListHandler.notifyUpdate(packet -> {});
        });
    ...
    private IWidget createListArea(PanelSyncManager syncManager, PanelSyncManager dynamicSyncManager) {
        ...
        if (clientConnections.isEmpty()) {
            return new TextWidget<>(StatCollector.translateToLocal("GT5U.gui.text.drone_no_machines"))
                    ...
                    Flow.row()
                        ...
                        .child(
                            new ItemDisplayWidget().background(IDrawable.EMPTY)
                                .displayAmount(false)
                                .item(connection.machineItem)
                                .size(16)
                                .tooltip(var -> getTooltipFromItemSafely(var, connection.machineItem)))
                        ...
                        .child(
                            connection.isSelected
                                ? createCustomNameTextField(connection, syncManager, dynamicSyncManager)
                                : new TextWidget<>(connection.getCustomName()).expanded()));
            }
            return droneListWidget;
        }
    }

    protected IWidget createCustomNameTextField(DroneConnection conn, PanelSyncManager syncManager,
        PanelSyncManager dynamicSyncManager) {
        DroneConnectionListSyncHandler droneConnectionListSyncHandler = syncManager
            .findSyncHandler("droneConnections", DroneConnectionListSyncHandler.class);
    ...
    public static IWidget createHighLightButton(DroneConnection connection, PanelSyncManager syncManager) {
        return new ButtonWidget<>().size(16)
            ...
            .onMousePressed(mouseButton -> {
                ...
                MTEDroneCentre.highlightMachine(player, connection.machineCoord);
                player.closeScreen();
                return true;
            })
            ...
            if (lastScrollY > 0) {
                if (getScrollArea().getScrollY() != null) {
                    getScrollArea().getScrollY()
                        .scrollTo(getScrollArea(), lastScrollY);
                }
                lastScrollY = 0;
            }
        }
