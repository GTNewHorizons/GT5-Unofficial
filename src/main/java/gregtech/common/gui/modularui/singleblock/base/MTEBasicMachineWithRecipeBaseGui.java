...
public class MTEBasicMachineWithRecipeBaseGui extends MTEBasicMachineBaseGui<MTEBasicMachineWithRecipe>{
    ...
    protected void registerSyncValues(PanelSyncManager syncManager) {
        ...
        BooleanSyncValue itemSync = new BooleanSyncValue(()-> machine.mItemTransfer, value -> machine.mItemTransfer = value);
        BooleanSyncValue fluidSync = new BooleanSyncValue(()-> machine.mFluidTransfer, value -> machine.mFluidTransfer = value);
    ...
    protected Flow createLeftCornerFlow(ModularPanel panel, PanelSyncManager syncManager) {
        ...
        if(!machine.isSteampowered()){
            cornerFlow.child(createAutoOutputButton("itemAutoOutput", GTGuiTextures.OVERLAY_BUTTON_AUTOOUTPUT_ITEM, BaseTileEntity.ITEM_TRANSFER_TOOLTIP));
            cornerFlow.child(createAutoOutputButton("fluidAutoOutput", GTGuiTextures.OVERLAY_BUTTON_AUTOOUTPUT_FLUID, BaseTileEntity.FLUID_TRANSFER_TOOLTIP));
        }
        return cornerFlow;
    }

    protected ToggleButton createAutoOutputButton(String syncKey, UITexture overlay, String tooltipKey){
        return new ToggleButton().size(18).syncHandler(syncKey).overlay(overlay).tooltip(t->t.addLine(GTUtility.translate(tooltipKey)));
    }
        ...
        return new ItemSlot().marginTop(4)
            .slot(
                new ModularSlot(machine.inventoryHandler, machine.getSpecialSlotIndex())
                    .slotGroup("item_inv")).tooltip(t->t.addLine(GTUtility.translate(key)));
    }
}
