...
public class RedstoneSnifferGuiBuilder {
    ...
    public ModularPanel build() {
        ...
        if (guiData.getMainHandItem().getTagCompound() != null
                && guiData.getMainHandItem().getTagCompound().hasKey("last_page")) {
        ...
        IntSyncValue pageSyncer = new IntSyncValue(lastPage::get, (page) -> {
            lastPage.set(page);
            if (guiData.getMainHandItem().getTagCompound() == null)
                guiData.getMainHandItem().setTagCompound(new NBTTagCompound());
        ...
        panel.flex().sizeRel(0.5f, 0.75f).align(Alignment.Center);
        ...
        regularMapSyncer.setChangeListener(() -> {
            if (!regularListWidget.getChildren().isEmpty()) return;
            ...
            entries.forEach(entry -> {
                ...
                regularList.add(
                        new Row().setEnabledIf(w -> (freqFilter.isEmpty() || entry.freq.equals(freqFilter)))
                                .sizeRel(1f, 0.1f * scale).expanded()
                                .background(
                                        bgStripe.get() % 2 == 0 ? new Rectangle().setColor(stripe1)
                                                : new Rectangle().setColor(stripe2))
                                .child(
                                        new TextWidget(entry.freq).widthRel(0.5f).color(textColor)
                                                .alignment(Alignment.Center))
                                .child(
                                        new TextWidget(
                                                entry.isPrivate ? IKey.lang("gui.yes").toString()
                                                        : IKey.lang("gui.no").toString()).widthRel(0.5f)
                                                                .color(textColor).alignment(Alignment.Center)));
            });
                ...
                new Column()
                        .child(
                                new Row().heightRel(0.1f)
                                        .child(
                                                new TextWidget(IKey.lang("gt.item.redstone_sniffer.frequency"))
                                                        .widthRel(0.5f).color(textColor).alignment(Alignment.Center))
                                        .child(
                                                new TextWidget(IKey.lang("gt.item.redstone_sniffer.private"))
                                                        .widthRel(0.5f).color(textColor).alignment(Alignment.Center)))
                        .child(new SingleChildWidget<>().sizeRel(1, 0.9f).child(regularListWidget)));
                            ...
                            coverMap.forEach((frequency, covers) -> {
                                covers.forEach(
                                        (coverPosition, ignored) -> {
                                            result.add(
                                                    new ItemRedstoneSniffer.SnifferEntry(
                                                            owner,
                                                            frequency,
                                                            coverPosition));
                                        });
        ...
        advancedMapSyncer.setChangeListener(() -> {
            if (!advancedListWidget.getChildren().isEmpty()) return;
        ...
        data.addPage(
                new Column()
                        .child(
                                new Row().heightRel(0.1f)
                                        .child(
                                                new TextWidget(IKey.lang("gt.item.redstone_sniffer.owner"))
                                                        .widthRel(0.15f).color(textColor).alignment(Alignment.Center))
                                        .child(
                                                new TextWidget(IKey.lang("gt.item.redstone_sniffer.frequency"))
                                                        .widthRel(0.35f).color(textColor).alignment(Alignment.Center))
                                        .child(
                                                new TextWidget(IKey.lang("gt.item.redstone_sniffer.dimension"))
                                                        .widthRel(0.25f).color(textColor).alignment(Alignment.Center))
                                        .child(
                                                new TextWidget(IKey.lang("gt.item.redstone_sniffer.action"))
                                                        .widthRel(0.25f).color(textColor).alignment(Alignment.Center)))
                        .child(new SingleChildWidget<>().sizeRel(1, 0.9f).child(advancedListWidget)));
        panel.child(
                new Column().margin(10).child(
                        new Row().heightRel(0.1f).marginBottom(10)
                                .child(
                                        new PageButton(0, controller).widthRel(0.5f).align(Alignment.CenterLeft)
                                                .overlay(IKey.lang("gt.item.redstone_sniffer.regular_wireless")))
                                .child(
                                        new PageButton(1, controller).widthRel(0.5f).align(Alignment.CenterRight)
                                                .overlay(IKey.lang("gt.item.redstone_sniffer.advanced_wireless"))))
                        .child(
                                new Row().heightRel(0.1f).marginBottom(10)
                                        .child(
                                                new TextWidget(IKey.lang("gt.item.redstone_sniffer.frequency_filter"))
                                                        .widthRel(0.25f).color(textColor).alignment(Alignment.Center))
                                        .child(
                                                new TextFieldWidget().sizeRel(0.25f, 0.5f).setTextColor(textColor)
                                                        .value(
                                                                SyncHandlers.string(
                                                                        () -> freqFilter,
                                                                        filter -> freqFilter = filter)))
                                        .child(
                                                new TextWidget(IKey.lang("gt.item.redstone_sniffer.owner_filter"))
                                                        .widthRel(0.25f).color(textColor).alignment(Alignment.Center))
                                        .child(
                                                new TextFieldWidget().sizeRel(0.25f, 0.5f).setTextColor(textColor)
                                                        .value(
    ...
    public List<IWidget> processAdvancedFrequencies(List<ItemRedstoneSniffer.SnifferEntry> entryList,
            ListWidget<IWidget, CategoryList.Root> listWidget, PanelSyncManager guiSyncManager, int scale,
            int textColor) {
        ...
        for (ItemRedstoneSniffer.SnifferEntry entry : entryList) {
            ...
            result.add(
                    new Row()
                            ...
                            .expanded()
                            .child(
                                    new TextWidget(entry.owner).widthRel(0.15f).color(textColor)
                                            .alignment(Alignment.Center))
                            .child(
                                    new TextWidget(entry.freq).widthRel(0.35f).color(textColor)
                                            .alignment(Alignment.Center))
                            .child(
                                    new TextWidget(cover.getDimName()).widthRel(0.25f).color(textColor)
                                            .alignment(Alignment.Center))
                            .child(
                                    new Row().widthRel(0.25f).child(
                                            new SingleChildWidget<>().widthRel(0.5f).child(
                                                    new ButtonWidget<>().size(25, 25).align(Alignment.Center).overlay(
                                                            UITexture
                                                                    .fullImage(
                                                                            GregTech.ID,
                                                                            "gui/overlay_button/redstoneSnifferLocate")
                                                                    .asIcon().size(19, 19).margin(3))
                                                            .tooltip(
                                                                    tooltip -> {
                                                                        tooltip.addLine(
                                                                                IKey.lang(
                                                                                        "gt.item.redstone_sniffer.locate"));
                                                                    })
                                                                    ...
                                                                    new ButtonWidget<>().size(25, 25)
                                                                            ...
                                                                            .tooltip(
                                                                                    tooltip -> {
                                                                                        tooltip.addLine(
                                                                                                IKey.lang(
                                                                                                        "gt.item.redstone_sniffer.teleport"));
                                                                                    })
                                                                            .onMousePressed(mouseButton -> {
                                                                                GTValues.NW.sendToServer(
                                                                                        new PacketDebugRedstoneCover(
