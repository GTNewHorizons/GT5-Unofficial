...
public class PowerGogglesGuiOverlay {
    ...
    public static void init() {
        ...
        OverlayManager.register(
                new OverlayHandler(
                        ...
                        screen -> new ModularScreen(ModularPanel.defaultPanel("overlay").size(0).pos(-100, 0))));
    }
    ...
    private static CustomModularScreen buildScreen(GuiScreen screen) {
        ...
        ModularPanel overlayPanel = ModularPanel
                .defaultPanel("power_goggles_overlay", gui.displayWidth, gui.displayHeight).size(230 + 2, height)
                .background(IDrawable.EMPTY).leftRel(0.5f).top(10);
        ...
        }.controller(controller).addPage(makeGeneralSettingsPage(gui)).addPage(makeColorSchemePage(overlayPanel));
            ...
            public @NotNull ModularPanel buildUI(ModularGuiContext context) {
                return overlayPanel.child(
                        new Column().sizeRel(1)
                                .child(
                                        new SingleChildWidget<>().size(230, 22).background(background).height(22)
                                                .paddingBottom(4).child(makePagedWidgetButton(controller)))
                                .child(new SingleChildWidget<>().size(230, height - 22).child(pagedWidget.sizeRel(1))));
            }
        };
    }
    ...
    private static IWidget makePagedWidgetButton(PagedWidget.Controller controller) {
        return new ButtonWidget<>().overlay(new DynamicDrawable(() -> IKey.lang(settings[settingsPage])))
                .onMousePressed(mouseButton -> {
                    ...
                    return true;
                }).sizeRel(1).align(Alignment.Center);
    }

    private static IWidget makeGeneralSettingsPage(PowerGogglesGuiHudConfig gui) {
        return new Column().coverChildren().background(background).child(makeNotationButton(gui))
                .child(makeReadingButton(gui)).child(makeChatHidesHudButton())
                .child(
                        makeSliderFlow(
    ...
    private static IWidget makeNotationButton(PowerGogglesGuiHudConfig gui) {
        ...
        notationButton.onMousePressed(mouseButton -> {
            ...
            PowerGogglesConfigHandler.config.getCategory(Configuration.CATEGORY_GENERAL).get("Format Index")
        ...
        return notationButton.size(230, 18).marginBottom(4);
    }
    ...
    private static IWidget makeReadingButton(PowerGogglesGuiHudConfig gui) {
        ...
        readingButton.onMousePressed(mouseButton -> {
            ...
            PowerGogglesConfigHandler.config.getCategory(Configuration.CATEGORY_GENERAL).get("Reading Index")
        ...
        return readingButton.size(230, 18).marginBottom(4);
    }
    ...
    private static IWidget makeChatHidesHudButton() {
        ...
        chatHidesHudButton.onMousePressed(mouseButton -> {
            ...
            PowerGogglesConfigHandler.config.getCategory(Configuration.CATEGORY_GENERAL).get("Hide HUD")
        ...
        return chatHidesHudButton.size(230, 18).marginBottom(4);
    }

    private static IWidget makeSliderFlow(Supplier<Double> valSupplier, Consumer<Double> setter, String key,
            String textKey) {
        return new Row().size(230, 18).marginBottom(4)
                .child(
                        IKey.lang(textKey).color(Color.WHITE.main).asWidget().paddingLeft(3).width(106).marginRight(14)
                                .alignment(Alignment.CenterRight))
                .child(
                        new SliderWidget().size(110, 18).background(GuiTextures.MC_BUTTON).bounds(0, 2).stopper(0.1)
    ...
    private static IWidget makeColorSchemePage(ModularPanel overlayPanel) {
        return new Column().coverChildren().background(background).align(Alignment.TopCenter)
                .child(
                ...
                .child(
                        new Row().size(228, 18).marginBottom(4).child(
                                new DynamicDrawable(
                                        () -> new Rectangle().setHorizontalGradient(
                                                PowerGogglesConfigHandler.gradientBadColor,
                                                PowerGogglesConfigHandler.gradientOkColor)).asWidget().size(114, 18))
                                .child(
                                        new DynamicDrawable(
                                                () -> new Rectangle().setHorizontalGradient(
                                                        PowerGogglesConfigHandler.gradientOkColor,
    ...
    private static IWidget makeColorConfigButton(ModularPanel overlayPanel, Supplier<Integer> colorSupplier,
        ...
        IPanelHandler colorPicker = IPanelHandler
                .simple(overlayPanel, (bla, blab) -> new ColorPickerDialog(key, val -> {
                    setter.accept(val);
                    PowerGogglesConfigHandler.config.getCategory(Configuration.CATEGORY_GENERAL).get(key).set(val);
                    PowerGogglesConfigHandler.config.save();
                }, colorSupplier.get(), true).size(200, 100), true);

        return new Row().size(230, 18).marginBottom(4)
                .child(
                        IKey.lang(buttonKey).color(Color.WHITE.main).asWidget().size(106, 18).marginRight(14)
                                .alignment(Alignment.CenterRight))
                .child(
                        new ButtonWidget<>().size(18, 18)
                                ...
                                .tooltipBuilder(t -> t.addLine(IKey.lang((tooltipKey)))).onMousePressed(d -> {
