...
/**
 * A standalone, single-purpose wrench item.
 * <p/>
 * Unlike {@link MetaGeneratedTool}, where the metadata is the tool type and the material lives in NBT, this item's
 ...
 * which stores energy instead of durability.
 */
...
public class ToolWrenchItem extends GTGenericItem implements IGTTool, IDamagableItem, IToolWrench, IAEWrench, ITool {

    /** Where the accumulated durability damage is kept, in the unit where 100 is one durability point. */
    private static final String DAMAGE_KEY = "GT.ToolDamage";
    /** Where the selected tool mode is kept. */
    private static final String MODE_KEY = "GT.ToolMode";
    ...
    /** The metadata values this wrench has actually been registered for, in registration order. */
    ...
    public ToolWrenchItem(String unlocalizedName, IToolStats toolStats, String englishNameFormat,
        ...
        GregTechAPI.sWrenchList.add(new GTItemStack(this, 1, OreDictionary.WILDCARD_VALUE));
    ...
    /**
     * Declares that this wrench can be made from the given material, which registers the resulting stack with the ore
    ...
    public ItemStack registerMaterial(Materials material, int meta, TC_AspectStack... aspects) {
        ...
        if (meta < 0) {
            // No metadata slot means no wrench for this material, which would quietly drop a recipe -- say so.
        ...
        GTToolItems
            .registerOreDictEntry(() -> GTOreDictUnificator.registerOre(ToolDictNames.craftingToolWrench, stack));
    ...
    /**
     * @return the metadata this material's wrench uses, or -1 if the material has no metadata slot at all.
     */
    ...
    /**
     * @return a freshly crafted wrench of this material, or null if the material has no metadata slot.
     */
    ...
    /**
     * @return the generic stats shared by every wrench of this tier, independent of any particular stack.
     */
    ...
    public void registerIcons(IIconRegister iconRegister) {
        // Rendered from the material's icon set by ToolItemRenderer, so there is no icon of our own to register.
    }
