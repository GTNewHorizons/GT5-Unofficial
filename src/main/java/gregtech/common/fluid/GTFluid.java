...
public class GTFluid extends Fluid implements IGTFluid, IGTRegisteredFluid, Runnable {
    ...
    public IGTRegisteredFluid configureMaterials(final Materials material) {
        if (material != null) configureMaterialSlots(material, MU.material(material));
        return this;
    }

    @Override
    public IGTRegisteredFluid configureMaterials(final Material material) {
        if (material != null) configureMaterialSlots(MU.materialOf(material), material);
        return this;
    }

    /// Writes the legacy facade fluid field and `Materials#FLUID_MAP` entry (when `legacy` is non-null), plus
    /// `MU`'s slot store and `Fluid`->`Material` twin map (when `ml` is non-null), for the state this fluid
    /// carries. The MU twin only carries ML-backed materials; a facade-only material's entry lives solely in
    /// FLUID_MAP, whose readers null-check either way.
    ///
    /// In dump mode the legacy builders re-run even for materials whose fluid MaterialLib already wired (see
    /// GTProxy's addAutoGenerated* bypasses), purely so the constructor capture above fires; leaving an
    /// already-wired slot untouched keeps the dumped fluid fields ground truth (e.g. Concrete's hand-wired
    ...
    private void configureMaterialSlots(Materials legacy, Material ml) {
        ...
        if (legacy != null) {
            switch (fluidState) {
                ...
                case GAS -> legacy.mGas = registeredFluid;
                case PLASMA -> legacy.mPlasma = registeredFluid;
                case MOLTEN -> legacy.mStandardMoltenFluid = registeredFluid;
                default -> legacy.mFluid = registeredFluid;
            }
            Materials.FLUID_MAP.put(registeredFluid, legacy);
        }
        if (ml != null) {
