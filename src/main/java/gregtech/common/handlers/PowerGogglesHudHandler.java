...
public class PowerGogglesHudHandler {
    ...
    public void drawHUD(RenderGameOverlayEvent.Post event) {
        if (event.type != RenderGameOverlayEvent.ElementType.ALL || mc.gameSettings.showDebugInfo
                || (PowerGogglesConfigHandler.hideWhenChatOpen && mc.currentScreen instanceof GuiChat))
            return;
    ...
    private static int drawPowerRectangle(int xOffset, int yOffset, int w, int h, int screenHeight, int borderRadius) {
        ...
        double severity = measurement.compareTo(BigInteger.ZERO) == 0 ? 0
                : new BigDecimal(change5m.multiply(BigInteger.valueOf(100)))
                        .divide(new BigDecimal(measurement), RoundingMode.FLOOR).intValue() / 100f;
                        ...
                        toFormatted(
                                change5m.divide(
                                        measurements.isEmpty() ? BigInteger.ONE
                                                : BigInteger.valueOf(
                                                        Math.min(
                                                                measurements.size() * ticksBetweenMeasurements,
                                                                5 * MINUTES)))))
                        : "");
                                ...
                                change5m.divide(
                                        BigInteger.valueOf(
                                                Math.min(
                                                        ...
                                                        5 * MINUTES)))))
                        : "0 EU/t");
                change1hString = "1h: " + (change1hDiff != 0 ? String.format(
                        "(%s EU/t)",
                        toFormatted(
                                change1h.divide(
                                        BigInteger.valueOf(
        ...
        String percentage = (measurement.compareTo(BigInteger.ZERO) == 0 || highest.compareTo(BigInteger.ZERO) == 0)
                ...
                : String.format(
                        "%.2f%%",
                        100 * measurement.floatValue() / (capacity > 0 ? capacity : highest.floatValue()));
        drawScaledString(
                fontRenderer,
                storage,
                xOffset,
        ...
        drawScaledString(
                ...
                percentage,
                xOffset + (int) ((h
                        - fontRenderer.getStringWidth(percentage) * PowerGogglesConfigHandler.mainTextScaling)),
                screenHeight - yOffset - w - gapBetweenLines - (int) (fontRenderer.FONT_HEIGHT * mainScale),
                change5mColor,
                mainScale);
        drawScaledString(
    ...
    private void drawPowerChart(int xOffset, int yOffset, int chartWidth, int chartHeight, int screenHeight,
        ...
        minReading = minReading.compareTo(BigInteger.ZERO) == 0 ? BigInteger.ZERO
                : BigInteger.valueOf(10).pow(exponent);
        ...
        double lastY = (yOffset + (chartHeight * (-1 + Math.min(
                1,
                1 - lastReading.subtract(minReading).floatValue() / maxReading.subtract(minReading).floatValue()))));
        for (int i = 1; i < readings; i++) {
            ...
            double y = (yOffset + (chartHeight * (-1 + Math.min(
                    1,
                    1 - reading.subtract(minReading).floatValue() / maxReading.subtract(minReading).floatValue()))));
    ...
    public static int[] getGradient(double severity, double scale, int gradientLeft, int gradientRight) {
        ...
        int newLeftGreen = Math.min(
                255,
                Math.max(0, Color.getGreen(gradientRight) + (int) (diffGreen * Math.min(1, severity * scale))));
                ...
                + (change5mDiff != 0 ? String.format(
                        ...
                        toFormatted(
                                change5m.divide(
                                        BigInteger.valueOf(
                                                Math.min(
                                                        Math.min(1, measurements.size() * ticksBetweenMeasurements),
                                                        5 * MINUTES)))))
                        : "");
        change1hString = "1h: " + toFormatted(change1h)
                + " EU "
