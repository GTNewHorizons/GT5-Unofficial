...
public class CoverIOBaseGui extends CoverGui<CoverIOBase> {
    ...
    public void addUIWidgets(PanelSyncManager syncManager, Flow column) {
        ...
        IWidget exportImportButtons = new EnumRowBuilder<>(TransferMode.class).value(ioModeSyncValue)
                .overlay(GTGuiTextures.OVERLAY_BUTTON_EXPORT, GTGuiTextures.OVERLAY_BUTTON_IMPORT).build();
        IWidget exportImportLabel = IKey.str(GTUtility.trans("229", "Export/Import")).asWidget();
        ...
        IWidget conditionButtons = new EnumRowBuilder<>(MachineProcessingCondition.class).value(conditionModeSyncValue)
                ...
                .build();
        IWidget conditionLabel = IKey.str(GTUtility.trans("230", "Conditional")).asWidget();
        ...
        column.child(
                new Grid().marginLeft(WIDGET_MARGIN).coverChildren().minElementMarginRight(WIDGET_MARGIN)
                        ...
                        .alignment(Alignment.CenterLeft).row(exportImportButtons, exportImportLabel)
                        .row(conditionButtons, conditionLabel).row(blockingButtons, blockingLabel));
    }
}
