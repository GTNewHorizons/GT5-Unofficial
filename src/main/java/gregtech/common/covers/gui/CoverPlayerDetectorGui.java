    ...
    public void addUIWidgets(PanelSyncManager syncManager, Flow column) {
        ...
        column.child(
                new Grid().marginLeft(WIDGET_MARGIN).coverChildren().minElementMarginRight(WIDGET_MARGIN)
                        ...
                        .row(
                                new SelectButton()
                                        .value(LinkedBoolValue.of(modeSyncValue, PlayerDetectionMode.ANY_PLAYER))
                                        .overlay(true, GTGuiTextures.OVERLAY_BUTTON_CHECKMARK)
                                        .addTooltipLine(
                                                IKey.str(GTUtility.trans("068.1", "Emit if any Player is close")))
                                        .size(16),
                                IKey.str(GTUtility.trans("319", "Any player")).asWidget())
                        .row(
                                new SelectButton()
                                        .value(LinkedBoolValue.of(modeSyncValue, PlayerDetectionMode.OTHER_PLAYERS))
                                        .overlay(true, GTGuiTextures.OVERLAY_BUTTON_CHECKMARK)
                                        .addTooltipLine(
                                                IKey.str(GTUtility.trans("069.1", "Emit if other Player is close")))
                                        .size(16),
                                IKey.str(GTUtility.trans("320", "Other players")).asWidget())
                        .row(
                                new SelectButton()
                                        .value(LinkedBoolValue.of(modeSyncValue, PlayerDetectionMode.ONLY_OWNER))
                                        .overlay(true, GTGuiTextures.OVERLAY_BUTTON_CHECKMARK)
                                        .addTooltipLine(IKey.str(GTUtility.trans("070", "Emit if you are close")))
                                        .size(16),
                                IKey.str(GTUtility.trans("321", "Only owner")).asWidget()));
    }
}
