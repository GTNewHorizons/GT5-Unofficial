...
public class CoverItemFilterGui extends CoverGui<CoverItemFilter> {
    ...
    public void addUIWidgets(PanelSyncManager syncManager, Flow column) {
        ...
        column.child(
                Flow.column().coverChildren().crossAxisAlignment(Alignment.CrossAxis.START).marginLeft(WIDGET_MARGIN)
                        .child(
                                Flow.row().coverChildren().childPadding(WIDGET_MARGIN)
                                        .child(
                                                new EnumRowBuilder<>(FilterType.class).value(filterTypeSyncValue)
                                                        ...
                                                        .build())
                                        .child(IKey.str(GTUtility.trans("318", "Check Mode")).asWidget()))
                        .child(IKey.str(GTUtility.trans("317", "Filter: ")).asWidget().marginTop(WIDGET_MARGIN)).child(
