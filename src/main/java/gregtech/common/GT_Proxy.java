...
//            GT_FML_LOGGER.info("Thingy Name: "+ aOre.mEvent.Name+ " !!!Unknown 'Thingy' detected!!! This Object seems to probably not follow a valid OreDictionary Convention, or I missed a Convention. Please report to GregTech Intergalactical for additional compatiblity. This is not an Error, an Issue nor a Lag Source, it is just an Information, which you should pass to me.");
    ...
    public Fluid addAutoGeneratedCorrespondingFluid(Materials aMaterial){
        // If the fluid is registered as custom inside the Material's constructor then this will trigger instead.
        // To add custom fluid textures go to blocks/fluids and place the .png. File should be called fluid.fluid.{filename}.png.
        if(aMaterial.mIconSet.is_custom) {
            return addFluid("molten." + aMaterial.mName.toLowerCase(Locale.ENGLISH), "fluid." + aMaterial.mName.toLowerCase(), "Molten " + aMaterial.mDefaultLocalName, aMaterial,
    ...
    public Fluid addAutoGeneratedCorrespondingGas(Materials aMaterial) {
        // If the fluid is registered as custom inside the Material's constructor then this will trigger instead.
        // To add custom fluid textures go to blocks/fluids and place the .png. File should be called fluid.gas.{filename}.png.
        if(aMaterial.mIconSet.is_custom) {
            return addFluid(aMaterial.mName.toLowerCase(Locale.ENGLISH), "gas." + aMaterial.mName.toLowerCase(), aMaterial.mDefaultLocalName, aMaterial,
    ...
    public Fluid addAutogeneratedMoltenFluid(Materials aMaterial) {
        // If the fluid is registered as custom inside the Material's constructor then this will trigger instead.
        // To add custom fluid textures go to blocks/fluids and place the .png. File should be called fluid.molten.{filename}.png.
        if(aMaterial.mIconSet.is_custom) {
            return addFluid("molten." + aMaterial.mName.toLowerCase(Locale.ENGLISH), "molten." + aMaterial.mName.toLowerCase(), "Molten " + aMaterial.mDefaultLocalName, aMaterial,
                aMaterial.mMoltenRGBa, 4, aMaterial.mMeltingPoint <= 0 ? 1000 : aMaterial.mMeltingPoint, GT_OreDictUnificator.get(OrePrefixes.cellMolten, aMaterial, 1L), ItemList.Cell_Empty.get(1L), 144);
        }
    ...
    public Fluid addAutogeneratedPlasmaFluid(Materials aMaterial) {
        // If the fluid is registered as custom inside the Material's constructor then this will trigger instead.
        // To add custom fluid textures go to blocks/fluids and place the .png. File should be called fluid.plasma.{filename}.png.
        if(aMaterial.mIconSet.is_custom) {
            return addFluid("plasma." + aMaterial.mName.toLowerCase(Locale.ENGLISH), "plasma." + aMaterial.mName.toLowerCase(), aMaterial.mDefaultLocalName + " Plasma", aMaterial,
...
//        String test = aTexture;
//        if (aName.toLowerCase().contains("spacetime")){
//            test = "molten.spacetime";
//            System.out.println("Confirmed if statement tripped." + aTexture + " " + aTemperatureK);
