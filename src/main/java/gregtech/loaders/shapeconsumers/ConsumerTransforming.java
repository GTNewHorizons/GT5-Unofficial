...
/// shape via [MaterialParts#shapes]. Every candidate shape of a matching prefix is delegated, so the fluid
/// and item pipe shapes sharing a `pipeTiny`..`pipeHuge` prefix each dispatch their own materials. `plank`
/// (which [ProcessingTransforming] additionally remaps to `plate` internally) has no MaterialLib shape and is
/// skipped like every other prefix [MaterialParts#shapes] does not resolve.
