...
/// Reconstructs legacy {@link Materials} instances from their MaterialLib-ported data, so
...
///
/// Known gaps, none of which are represented in MaterialLib's ported data and are small enough in scope to
...
public class LegacyMaterials {
    ...
    public static Materials build(Material ml) {
        ...
        // present): MaterialLib's ported data has no representation for a gas-conditional
    ...
    /// material, in place of the per-material `GTFluidFactory` construction in `GTProxy`'s `addAutoGenerated*`
    /// methods (those no-op for a material whose fields are already wired here, since MaterialLib --
    /// resolving before GT's own preInit -- always registers first).
