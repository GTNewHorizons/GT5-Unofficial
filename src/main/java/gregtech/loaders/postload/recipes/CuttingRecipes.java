...
public class CuttingRecipes implements Runnable {
    ...
    public void recipeWithPurifiedWater(ItemStack[] inputs, ItemStack[] outputs, Materials lowTierWater,
        ...
        GTValues.RA.stdBuilder().itemInputs(inputs).itemOutputs(outputs).fluidInputs(lowTierWater.getFluid(100L))
                .duration(duration).eut(eut).addTo(cutterRecipes);
        // Bonus for using higher tier water
        GTValues.RA.stdBuilder().itemInputs(inputs).itemOutputs(outputs).fluidInputs(highTierWater.getFluid(100L))
                .duration(boostedDuration).eut(eut).addTo(cutterRecipes);
    }
    ...
    public void recipeWithClassicFluids(ItemStack[] inputs, ItemStack[] outputs, int duration, long eut,
        ...
        if (cleanroomRequired) {
            GTValues.RA.stdBuilder().itemInputs(inputs).itemOutputs(outputs)
                    .fluidInputs(Materials.Water.getFluid(Math.max(4, Math.min(1000, duration * eut / 320))))
                    .duration(2 * duration).eut(eut).requiresCleanRoom().addTo(cutterRecipes);
            ...
            GTValues.RA.stdBuilder().itemInputs(inputs).itemOutputs(outputs)
                    ...
                    .duration(duration).eut(eut).requiresCleanRoom().addTo(cutterRecipes);
        } else {
            GTValues.RA.stdBuilder().itemInputs(inputs).itemOutputs(outputs)
                    .fluidInputs(Materials.Water.getFluid(Math.max(4, Math.min(1000, duration * eut / 320))))
                    .duration(2 * duration).eut(eut).addTo(cutterRecipes);
