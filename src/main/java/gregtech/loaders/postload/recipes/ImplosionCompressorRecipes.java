...
public class ImplosionCompressorRecipes implements Runnable {
    ...
    @Override
    public void run() {
        GTValues.RA.stdBuilder().itemInputs(ItemList.IC2_Compressed_Coal_Chunk.get(1L))
                .itemOutputs(
                        ItemList.IC2_Industrial_Diamond.get(1L),
                        GTOreDictUnificator.get(OrePrefixes.dustTiny, Materials.DarkAsh, 4L))
                .metadata(GTRecipeConstants.ADDITIVE_AMOUNT, 8).duration(1 * SECONDS).eut(TierEU.RECIPE_LV)
                .addTo(implosionRecipes);

        GTValues.RA.stdBuilder().itemInputs(ItemList.Ingot_IridiumAlloy.get(1L))
                .itemOutputs(
        ...
        if (GalacticraftMars.isModLoaded()) {
            ...
            GTValues.RA.stdBuilder().itemInputs(ItemList.Ingot_Heavy1.get(1L))
                    .itemOutputs(
            ...
            GTValues.RA.stdBuilder().itemInputs(ItemList.Ingot_Heavy2.get(1L))
                    .itemOutputs(
                            getModItem(GalacticraftMars.ID, "item.null", 1L, 3),
                            GTOreDictUnificator.get(OrePrefixes.dustTiny, Materials.TungstenSteel, 2L))
                    .metadata(GTRecipeConstants.ADDITIVE_AMOUNT, 16).duration(1 * SECONDS).eut(TierEU.RECIPE_LV)
                    .addTo(implosionRecipes);

            GTValues.RA.stdBuilder().itemInputs(ItemList.Ingot_Heavy3.get(1L))
                    .itemOutputs(
                    ...
                    .metadata(GTRecipeConstants.ADDITIVE_AMOUNT, 24).duration(1 * SECONDS).eut(TierEU.RECIPE_LV)
                    .addTo(implosionRecipes);
        }
    }
}
