...
public class PrinterRecipes implements Runnable {
    ...
    public void run() {
        GTValues.RA.stdBuilder().itemInputs(GTOreDictUnificator.get(OrePrefixes.plateDouble, Materials.Paper, 1L))
                ...
                .duration(5 * SECONDS).eut(2).addTo(printerRecipes);

        GTValues.RA.stdBuilder().itemInputs(ItemList.Paper_Punch_Card_Empty.get(1L))
        ...
        GTValues.RA.stdBuilder().itemInputs(GTOreDictUnificator.get(OrePrefixes.plate, Materials.Paper, 3L))
                .special(ItemList.Tool_DataStick.getWithName(0L, "With Scanned Book Data"))
                .itemOutputs(ItemList.Paper_Printed_Pages.get(1L)).fluidInputs(getFluidStack("squidink", 1 * INGOTS))
                .duration(20 * SECONDS).eut(2).addTo(printerRecipes);

        GTValues.RA.stdBuilder().itemInputs(new ItemStack(Items.map, 1, 32767))
                .special(ItemList.Tool_DataStick.getWithName(0L, "With Scanned Map Data"))
                .itemOutputs(new ItemStack(Items.filled_map, 1, 0)).fluidInputs(getFluidStack("squidink", 1 * INGOTS))
                .duration(20 * SECONDS).eut(2).addTo(printerRecipes);

        GTValues.RA.stdBuilder().itemInputs(new ItemStack(Items.book, 1, 32767))
                .itemOutputs(GTUtility.getWrittenBook("Manual_Printer", ItemList.Book_Written_01.get(1L)))
                .fluidInputs(getFluidStack("squidink", 1 * INGOTS)).duration(20 * SECONDS).eut(2).addTo(printerRecipes);
    }
}
