...
public class LoaderGTItemData implements Runnable {
    ...
    public void run() {
        ...
        ImmutableMap.of(NewHorizonsCoreMod.ID, ImmutableMap.<String, Materials>builder()
            ...
            .build(), EnderIO.ID, ImmutableMap.<String, Materials>builder()
            ...
            .build()).forEach((modId, items) -> items.forEach((item, material) -> registerMetalBarUnificationData(modId, item, material)));
        GTOreDictUnificator
    ...
    private static void registerMetalBarUnificationData(String modId, String itemName, Materials material) {
        GTOreDictUnificator.addItemData(
            GTModHandler.getModItem(modId, itemName, 8L, 0),
            new ItemData(material, 10886400L));
    }
}
