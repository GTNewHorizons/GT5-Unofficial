...
public class LoaderGTBlockFluid implements Runnable {
    ...
    public void run() {
        ...
        // Autogenerate fluids for the legacy facade material set, iterated through the MaterialLib registry.
        // MU.isLegacyNamed selects exactly the materials Materials.values() covered (the facade<->ML name
        // bijection over MATERIALS_MAP, markers/superconductors excluded on both sides), so a registry material
        // with no facade twin -- a werkstoff or gtpp reconstruction -- is skipped rather than gaining an
        // autogenerated fluid it never had (a naive getMaterials() swap would triple the loop).
        for (Material material : MaterialLibAPI.getMaterials()) {
            if (!MU.isLegacyNamed(material)) continue;
            // GTFluid.DUMP_MODE bypasses these molten-wired checks (in addition to the skip inside
            // addAutogeneratedMoltenFluid itself) so the legacy builder actually runs and captures its texture
            // into GTFluid.DUMP_TEXTURES, even for materials MaterialLib already wired.
            if ((GTFluid.DUMP_MODE || !MU.hasMolten(material)) && MU.hasFlag(material, GTMaterialFlag.SMELTING_TO_FLUID)
                && !MU.hasFlag(material, GTMaterialFlag.NO_SMELTING)) {
                // mNotGeneratedItems is a legacy Materials-keyed blacklist with no MaterialLib twin.
                if (!OrePrefixes.cellMolten.mNotGeneratedItems.contains(MU.materialOf(material))) {
                    GTMod.proxy.addAutogeneratedMoltenFluid(material);
                }
                Material smeltInto = MU.smeltInto(material);
                if (smeltInto != material && (GTFluid.DUMP_MODE || !MU.hasMolten(smeltInto))) {
                    GTMod.proxy.addAutogeneratedMoltenFluid(smeltInto);
                }
            }
            if (MU.element(material) != null || (MU.hasPlasma(material) && !MU.iconSet(material).is_custom)) {
                GTMod.proxy.addAutogeneratedPlasmaFluid(material);
            }
            if (MU.hasCorrespondingFluid(material)) {
                GTMod.proxy.addAutoGeneratedCorrespondingFluid(material);
            }
            if (MU.hasCorrespondingGas(material)) {
