...
public class LoaderMetaTileEntities implements Runnable { // TODO CHECK CIRCUIT RECIPES AND USAGES
    ...
    private static void registerMultiblockControllers() {
        ...
        ItemList.LATEX.set(
            new MTELatex(LATEX.ID, "multimachine.latex", "L.A.T.E.X.").getStackForm(1));
        addItemTooltip(ItemList.LATEX.get(1), chain(() -> "Author: ", GTValues.AuthorThree));

        ItemList.SuperConductorProcessor.set(
            new MTESuperConductorProcessor(SUPERCONDUCTOR_PROCESSOR.ID, "multimachine.superconductor-processor", "SuperConductor Processor").getStackForm(1));
