...
public class ProcessingDirty implements gregtech.api.interfaces.IOreRecipeRegistrator {
    ...
    public void registerOreAsync(OrePrefixes aPrefix, Materials aMaterial, String aOreDictName, String aModName, ItemStack aStack) {
        ...
        createWasherRecipes(aPrefix, aMaterial, aStack);
        for (Materials tMaterial : aMaterial.mOreByProducts) {
            createWasherRecipes(aPrefix, tMaterial, aStack);
        }
    }

    private void createWasherRecipes(OrePrefixes aPrefix, Materials aMaterial, ItemStack aStack) {
