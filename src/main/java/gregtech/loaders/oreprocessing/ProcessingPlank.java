...
public class ProcessingPlank implements gregtech.api.interfaces.IOreRecipeRegistrator {
    ...
    /**
     * Sometimes automatically searching for recipes is not enough to ensure
     * that the corresponding slab is correctly assigned. These Special lists
     * exist to provide a direct mapping from plank to slab in such cases.
     *
     * Configuration works as follows:
     * Each nth entry in the SPECIAL_PLANKS lists corresponds to the same nth
     * entry in the SPECIAL_SLABS list.
     * Entries should consist of Strings formatted as:
     * "ModId:BlockId" OR "ModId:BlockId:Metadata"
     * The format for each entry in the SPECIAL_PLANKS defines the behaviour of
     * the entries in the SPECIAL_SLABS list.
     * - If the first entry is null or invalid, the entry is skipped.
     * (AKA it's a useless entry, there's no reason to do this)
     * - If the first entry doesn't have Metadata, ALL subtypes of this block will be
     * handled through the special case. The metadata is sourced from the ItemStack
     * defined in the {@link #convertSlabRecipe(ItemStack)} method call.
     * The ItemStack metadata will also be used for the second entry, UNLESS it
     * defines it's own metadata, in which case that will be used instead.
     * - If the first entry has Metadata, ONLY that specific block subtype will be
     * handled through the special case. This Metadata will be copied into the
     * second entry, UNLESS it defines it's own metadata, in which case that
     * will be used instead.
     * Additionaly, if the second entry is null, then creation of the slab recipes
     * will be skipped entirely, even if an OreDict recipe exists. The standard slab
     * recipes (be they from OreDict or not) will still be removed, however.
     *
     * Some examples of Mods which need special handling, as well as the reasons why:
     *
     * Witchery: The mod forgot to add the "standard" Plank -> Slab recipes, so we
     * need to manually assign them. In most cases this should instead be fixed in
     * the original mod, but we all know this ain't happening for Witchery.
     *
