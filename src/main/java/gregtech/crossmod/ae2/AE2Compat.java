...
public final class AE2Compat {
    ...
    private static void registerCover() {
        ItemStack facade = AEApi.instance().definitions().items().facade().maybeItem()
                .transform(i -> new ItemStack(i, 1, WILDCARD)).orNull();
        if (facade != null) {
            CoverRegistry.registerCover(
                    facade,
                    null,
                    CoverFacadeAE::new,
                    CoverPlacer.builder().allowOnPrimitiveBlock().onlyPlaceIf(CoverFacadeAE.isCoverPlaceable).build());
        }
    }

    private static void registerExternalStorage() {
        IExternalStorageRegistry registry = AEApi.instance().registries().externalStorage();
    ...
    private static void registerBlockingModeIgnoreItems() {
        IBlockingModeIgnoreItemRegistry registry = AEApi.instance().registries().blockingModeIgnoreItem();
        ...
        ItemList[] nonConsumables = new ItemList[] { ItemList.Shape_Mold_Bottle, ItemList.Shape_Mold_Plate,
                ...
                ItemList.Shape_Extruder_Pipe_Small, ItemList.Shape_Extruder_Pipe_Medium,
                ItemList.Shape_Extruder_Pipe_Large, ItemList.Shape_Extruder_Pipe_Huge, ItemList.Shape_Extruder_Block,
                ItemList.Shape_Extruder_Sword, ItemList.Shape_Extruder_Pickaxe, ItemList.Shape_Extruder_Shovel,
                ItemList.Shape_Extruder_Axe, ItemList.Shape_Extruder_Hoe, ItemList.Shape_Extruder_Hammer,
                ItemList.Shape_Extruder_File, ItemList.Shape_Extruder_Saw, ItemList.Shape_Extruder_Gear,
                ItemList.Shape_Extruder_Rotor, ItemList.Shape_Extruder_Turbine_Blade,
                ItemList.Shape_Extruder_Small_Gear, ItemList.Shape_Extruder_ToolHeadDrill,
