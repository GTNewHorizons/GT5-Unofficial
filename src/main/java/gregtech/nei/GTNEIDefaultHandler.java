...
public class GTNEIDefaultHandler extends TemplateRecipeHandler {
        ...
        if (cacheHolder.getCachedRecipesVersion() != GTMod.proxy.getNEIReloadCount()
            ...
            try {
                ...
                if (this.recipeCategory == defaultCategory) {
                    ...
                    Stream<GTRecipe> recipesToMerge = recipeMap.getBackend().getRecipeCategoryMap().entrySet().stream()
                            .flatMap(entry -> {
                    ...
                    recipes = Stream
                            .concat(
                                    recipesToMerge,
                                    recipeMap.getBackend().getRecipesByCategory(defaultCategory).stream())
                            .collect(Collectors.toList());
                } else {
                    // This is "sub" category
                    if (recipeCategory.settingSupplier.get() == RecipeCategorySetting.ENABLE) {
                        recipes = recipeMap.getBackend().getRecipesByCategory(recipeCategory);
                    } else {
                        recipes = Collections.emptyList();
                    }
                }
                cache = recipes.stream() // do not use parallel stream. This is already parallelized by NEI
                        .filter(r -> !r.mHidden).sorted(neiProperties.comparator).map(CachedDefaultRecipe::new)
                        .collect(Collectors.toList());
    ...
    private List<CachedDefaultRecipe> getTieredRecipesWithCustomFilter(OverclockDescriber overclockDescriber) {
        ...
        if (!recipes.isEmpty()) {
            recipes = recipes.stream().filter(recipe -> overclockDescriber.canHandle(recipe.mRecipe))
                    .collect(Collectors.toList());
        }
        return recipes;
    }
            ...
            for (Widget child : modularWindow.getChildren()) {
                if (child instanceof SlotWidget widget) {
                    if (widget.getMcSlot().getItemHandler() == itemInputsInventory) {
                        ...
                        int i = widget.getMcSlot().getSlotIndex();
                        ItemStack[] outputs = GTNEIDefaultHandler.this.neiProperties.itemOutputsGetter.apply(aRecipe);
                        ...
                        int i = widget.getMcSlot().getSlotIndex();
                        FluidStack[] outputs = GTNEIDefaultHandler.this.neiProperties.fluidOutputsGetter.apply(aRecipe);
