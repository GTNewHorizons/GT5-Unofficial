...
public class MTELINAC extends MTEEnhancedMultiBlockBase<MTELINAC> implements ISurvivalConstructable {
    ...
    static {
        STRUCTURE_DEFINITION = StructureDefinition.<MTELINAC>builder().addShape(
                ...
                .addElement(
                        ...
                        buildHatchAdder(MTELINAC.class).hatchClass(MTEHatchInputBeamline.class)
                                .casingIndex(CASING_INDEX).dot(3).adder(MTELINAC::addBeamLineInputHatch).build())
                .addElement(
                        'o',
                        buildHatchAdder(MTELINAC.class).hatchClass(MTEHatchOutputBeamline.class)
                                .casingIndex(CASING_INDEX).dot(4).adder(MTELINAC::addBeamLineOutputHatch).build())
                ...
                .addElement(
                        'h',
                        buildHatchAdder(MTELINAC.class).atLeast(InputHatch, OutputHatch).casingIndex(CASING_INDEX)
                                .dot(2).build())
                .addElement(
                        'j',
                        buildHatchAdder(MTELINAC.class).atLeast(Maintenance, Energy).casingIndex(CASING_INDEX).dot(1)
    ...
    protected MultiblockTooltipBuilder createTooltip() {
        ...
        tt.addMachineType("Particle Accelerator").addInfo("Controller block for the LINAC")
                ...
                .addInfo(DescTextLocalization.BEAMLINE_SCANNER_INFO).addInfo("Valid Coolants:");
        ...
        tt.addInfo("Requires (length)kL/s of coolant").beginVariableStructureBlock(7, 7, 7, 7, 19, 83, false)
                ...
                .addCasingInfoExactly("Any Tiered Glass (LuV+)", 48, false).addEnergyHatch(addDotText(1))
