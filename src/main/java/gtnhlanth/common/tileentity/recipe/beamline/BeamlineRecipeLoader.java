...
public class BeamlineRecipeLoader {
    ...
    public static void load() {
        ...
        coolantMap.put(
                Materials.LiquidNitrogen.getGas(1L).getFluid().getName(),
                Materials.Nitrogen.getGas(1L).getFluid());
        ...
        coolantMap.put(
                Materials.SuperCoolant.getFluid(1L).getFluid().getName(),
                WerkstoffMaterialPool.HotSuperCoolant.getFluidOrGas(1_000).getFluid());

        /*
         * ELECTRON
         */
        GTValues.RA.stdBuilder().itemInputs(GTOreDictUnificator.get(OrePrefixes.stick, Materials.Tungsten, 1))
                .metadata(
                        SOURCE_CHAMBER_METADATA,
                        SourceChamberMetadata.builder().particleID(Particle.ELECTRON.ordinal()).rate(20)
                                .energy(1000, 0.1f).focus(98).build())
                .duration(20).eut(TierEU.RECIPE_IV).addTo(sourceChamberRecipes);

        GTValues.RA.stdBuilder().itemInputs(WerkstoffMaterialPool.LanthanumHexaboride.get(OrePrefixes.stickLong, 1))
                .metadata(
                        SOURCE_CHAMBER_METADATA,
                        SourceChamberMetadata.builder().particleID(Particle.ELECTRON.ordinal()).rate(60)
                                .energy(5000, 0.3f).focus(99).build())
                .duration(20).eut(TierEU.RECIPE_IV).addTo(sourceChamberRecipes);

        /*
         * NEUTRON
         */
        GTValues.RA.stdBuilder().itemInputs(MaterialsElements.getInstance().CALIFORNIUM.getDust(1))
                .metadata(
                        SOURCE_CHAMBER_METADATA,
                        SourceChamberMetadata.builder().particleID(Particle.NEUTRON.ordinal()).rate(10)
                                .energy(9000, 999).focus(95).build())
                .duration(20).eut(TierEU.RECIPE_EV).addTo(sourceChamberRecipes);

        /*
         * ALPHA
         */
        GTValues.RA.stdBuilder().itemInputs(Materials.Uranium.getDust(1))
                .itemOutputs(WerkstoffMaterialPool.Thorium234.get(OrePrefixes.dust, 1))
                .metadata(
                        SOURCE_CHAMBER_METADATA,
                        SourceChamberMetadata.builder().particleID(Particle.ALPHA.ordinal()).rate(1).energy(4270, 999)
                                .focus(90).build())
                .duration(20).eut(TierEU.RECIPE_HV).addTo(sourceChamberRecipes);
        ...
        for (MaskList mask : MaskList.values()) {
            ...
            if (mask.getTCTargetItem() == null) { // Wafer TC recipe
                ...
                for (ItemList wafer : VIABLE_WAFERS) {
                    ...
                    if (!Arrays.asList(mask.getForbiddenWafers()).contains(wafer)) {
                        ...
                        GTValues.RA.stdBuilder().itemInputs(focusItem, wafer.get(1)).itemOutputs(
                                GTUtility
                                        .copyAmountUnsafe((int) GTUtility.powInt(2, index + 2), mask.getProducedItem()))
                                .metadata(
                                        TARGET_CHAMBER_METADATA,
                                        TargetChamberMetadata.builder(focusItem).particleID(1)
                                                ...
                                                .amount(
                                                        (int) Math.round(
                                                                mask.getBaselineAmount()
                                                                        * Math.sqrt(GTUtility.powInt(2, index - 1))))
                                                .energy(mask.getMinEnergy(), mask.getMaxEnergy(), 1)
                                                .minFocus(mask.getMinFocus()).build())
                                .duration(1).eut(1920).addTo(targetChamberRecipes);
                    }
                }
            ...
            ItemStack focusItem = new ItemStack(LanthItemList.maskMap.get(mask), 0);
            GTValues.RA.stdBuilder().itemInputs(focusItem, GTUtility.copyAmountUnsafe(1, mask.getTCTargetItem()))
                    .itemOutputs(GTUtility.copyAmountUnsafe(4, mask.getProducedItem()))
                    .metadata(
                            TARGET_CHAMBER_METADATA,
                            TargetChamberMetadata.builder(focusItem).particleID(1).amount(mask.getBaselineAmount())
                                    .energy(mask.getMinEnergy(), mask.getMaxEnergy(), 1).minFocus(mask.getMinFocus())
                                    .build())
                    .duration(1).eut(TierEU.RECIPE_LuV).addTo(targetChamberRecipes);
        }
        ...
        GTValues.RA.stdBuilder()
                ...
                .metadata(
                        TARGET_CHAMBER_METADATA,
                        TargetChamberMetadata.builder(focusItem).particleID(1).amount(24).energy(5, 12, 1).minFocus(60)
                                .build())
                .duration(1).eut(TierEU.RECIPE_LuV).addTo(targetChamberRecipes);
    }
}
