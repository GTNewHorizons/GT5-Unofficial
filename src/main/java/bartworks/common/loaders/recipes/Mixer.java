...
public class Mixer implements Runnable {
    ...
    public void run() {
        ...
        for (MTEHighTempGasCooledReactor.HTGRMaterials.Fuel_ fuel : MTEHighTempGasCooledReactor.HTGRMaterials.sHTGR_Fuel) {
            GTValues.RA.stdBuilder().itemInputs(fuel.mainItem, fuel.secondaryItem, GTUtility.getIntegratedCircuit(1))
        ...
        if (Gendustry.isModLoaded()) {
            GTValues.RA.stdBuilder()
                    ...
                    .itemOutputs(Materials.Empty.getCells(1)).fluidInputs(GTModHandler.getLiquidDNA(1_000))
                    .fluidOutputs(new FluidStack(FluidLoader.BioLabFluidMaterials[0], 2_000)).duration(25 * SECONDS)
